...
持久化的 `approval/asked` 事件会暂停当前活跃 timer 并保留精确剩余时长，最后一个匹配的 `approval/decided` 事件在所有待审批项结束后恢复该剩余时长。人工响应延迟不会消耗模型执行时间，审批结束后专家仍只拥有原执行窗口的剩余预算
...
专家可以在串行任务的完整等待期内保持 idle 或等待用户审批而不丢失成员资格，真正卡死的活跃执行仍会在配置 timeout 到期时关闭式失败。已有不可变任务历史不会被改写。runtime 重载后，已经处于 running 的 child 会在下一次 pre-step 获得替代 timer，不重建此前已经过去的进程内时间
