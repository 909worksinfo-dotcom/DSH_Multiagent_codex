...
describe('SkillMarketplace', () => {
  ...
  it('prepares a public Smithery connection before exposing an executable capability', async () => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (url === 'https://connect.example/connect/dsh-demo/huggingface/mcp' && init?.method === 'POST') {
        const body = JSON.parse(requestBody(init)) as { method: string; params?: { name?: string } }
  ...
  it('re-resolves stored marketplace credentials for search, prepare, and execute without a restart', async () => {
    const credentials = new Map<string, string>()
    const authorizationHeaders: string[] = []
    const composioHeaders: string[] = []
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      const url = requestUrl(input)
      const headers = init?.headers as Record<string, string> | undefined
      if (url.startsWith('https://smithery.example/servers?')) {
        if (headers?.Authorization !== undefined) authorizationHeaders.push(headers.Authorization)
        return Response.json({ servers: [{
          qualifiedName: 'evidence-search', displayName: 'Evidence Search', description: 'Search primary sources',
          verified: true, remote: true, isDeployed: true, inactive: false, useCount: 20,
        }] })
      }
      if (url === 'https://smithery.example/servers/evidence-search') {
        authorizationHeaders.push(headers?.Authorization ?? '')
        return Response.json({
          qualifiedName: 'evidence-search', displayName: 'Evidence Search',
          deploymentUrl: 'https://evidence.example/mcp',
          connections: [{ deploymentUrl: 'https://evidence.example/mcp', configSchema: {} }],
        })
      }
      if (url === 'https://connect.example/connect/dsh-demo/evidence-search' && init?.method === 'PUT') {
        authorizationHeaders.push(headers?.Authorization ?? '')
        return Response.json({ connectionId: 'evidence-search', status: { state: 'connected' } })
      }
      if (url === 'https://connect.example/connect/dsh-demo/evidence-search/mcp' && init?.method === 'POST') {
        authorizationHeaders.push(headers?.Authorization ?? '')
        const body = JSON.parse(requestBody(init)) as { method: string }
        return body.method === 'tools/list'
          ? Response.json({ result: { tools: [{ name: 'search' }] } })
          : Response.json({ result: { content: [{ type: 'text', text: 'evidence' }] } })
      }
      if (url.startsWith('https://composio.example/tools')) {
        composioHeaders.push(headers?.['x-api-key'] ?? '')
    ...
    const marketplace = await service({
      smitheryApiKeyEnv: 'SMITHERY_API_KEY',
      smitheryConnectEndpoint: 'https://connect.example/',
      smitheryNamespace: 'dsh-demo',
    ...
    const before = await marketplace.search('evidence research')
    expect(before.providers.find(provider => provider.source === 'composio')).toMatchObject({
      state: 'authorization_required', capabilities: [],
    })
    expect(authorizationHeaders).toEqual([])
    expect(composioHeaders).toEqual([])

    credentials.set('SMITHERY_API_KEY', 'stored-smithery-key')
    credentials.set('COMPOSIO_API_KEY', 'stored-composio-key')
    const after = await marketplace.search('evidence research')
    const candidate = after.providers.find(provider => provider.source === 'smithery')?.capabilities[0]
    if (candidate === undefined) throw new Error('Smithery candidate was not discovered')
    const connected = await marketplace.prepare(candidate)
    credentials.set('SMITHERY_API_KEY', 'rotated-smithery-key')
    await marketplace.execute(connected, { tool: 'search', arguments: { query: 'agents' } })

    expect(composioHeaders).toEqual(['stored-composio-key'])
    expect(authorizationHeaders).toEqual([
      'Bearer stored-smithery-key',
      'Bearer stored-smithery-key',
      'Bearer stored-smithery-key',
      'Bearer stored-smithery-key',
      'Bearer rotated-smithery-key',
    ])
  })
})
