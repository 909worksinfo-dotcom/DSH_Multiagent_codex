...
const smitheryResponse = z.object({
...
const composioResponse = z.object({
...
const smitheryDetailsResponse = z.object({
  qualifiedName: z.string().min(1),
...
const smitheryConnectionResponse = z.object({
  connectionId: z.string().min(1),
  status: z.object({ state: z.string().min(1) }).passthrough(),
}).passthrough()

const mcpToolsListResponse = z.object({
  result: z.object({
    tools: z.array(z.object({ name: z.string().min(1) }).passthrough()).default([]),
...
export class SkillMarketplace extends Service {
  ...
  async prepare(
    ...
    const apiKey = this.config.smitheryApiKey?.trim()
  ...
  private async searchSmithery(query: string, signal?: AbortSignal): Promise<SkillMarketplaceProviderResult> {
    try {
      const parsed = smitheryResponse.parse(await fetchJson(
        ...
        this.config.smitheryApiKey?.trim()
          ? { headers: { Authorization: `Bearer ${this.config.smitheryApiKey.trim()}` } }
          : {},
      ))
  ...
  private async searchComposio(query: string, signal?: AbortSignal): Promise<SkillMarketplaceProviderResult> {
    const apiKey = this.config.composioApiKey?.trim()
