...
与通道无关的审批 seam。`ctx.approval.request(req)` 返回 `allowed-once`、`allowed-for-turn`、`rejected`、`cancelled` 或 `unavailable`；应答者缺失或失败时会以拒绝方式关闭。`allowed-for-turn` 只在当前 agent 轮次内授权匹配请求，Web UI 将该轮次称为“本任务”。确切事件签名见 [approval.md](../../../docs/subsystems/approval.md#cordis-surface) 的生成区块。
...
- **任务授权只在轮次内存中生效**：`allowed-for-turn` 按工具名称与消费方提供的 `taskKey` 精确匹配，在 `turn/end` 时失效，且不会在进程重启或服务重载后恢复。系统没有 `allow-always`、已记住的规则或撤销存储。
- **请求不携带工具参数**：应答者会看到工具名称、原因和可选调用 id；消费方只有在能安全归类相似请求时才另行提供不用于展示的 `taskKey`。ACP 机器通道要求调用 id，并会委托不含 id 的请求。
