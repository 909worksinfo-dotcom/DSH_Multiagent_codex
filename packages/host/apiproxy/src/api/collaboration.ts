...
export interface CollaborationWorkstreamView {
  ...
  /** Planned expert seat responsible for this workstream before provisioning. */
  readonly assigneeSlotId?: string
}
...
export interface CollaborationExpertBindingView {
  ...
  /** User-safe daily-agent infrastructure inherited by this expert. */
  readonly foundation?: {
    readonly model: {
...
export interface CollaborationApi {
  /** Create and persist an inspectable profile, plan, and charter without provisioning experts. */
  create(request: RpcRequest<{
    ...
    /** Exact daily-session model route selected for the Lead coordinator. */
    leadModel?: ModelSelection
    /** Exact daily-session model routes selected for reviewed expert slots. */
    expertModels?: readonly {
      readonly slotId: string
      readonly selection: ModelSelection
    }[]
  }>, signal: AbortSignal): Promise<RpcResponse<CollaborationRunView>>

  /** Confirm the persisted plan, provision its exact roster, and activate the TeamRun. */
  confirm(request: RpcRequest<{ runId: SessionId; requestId: string }>, signal: AbortSignal):
  Promise<RpcResponse<CollaborationRunView>>

  /** List recoverable local TeamRuns newest-first, resuming cold Leads when required. */
  list(request: RpcRequest<{ limit?: number }>): Promise<RpcResponse<{ runs: CollaborationRunView[] }>>
