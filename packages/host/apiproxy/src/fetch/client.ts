...
import {
  ...
  collaborationConfirmValueSchema,
...
export interface IApiClient {
  ...
  collaboration: {
    ...
    confirm(payload: RequestPayload<'collaboration.confirm'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'collaboration.confirm'>>>
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'collaboration.confirm': collaborationConfirmValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly collaboration: IApiClient['collaboration'] = {
    ...
    confirm: (payload, signal) => this.callUnary('collaboration.confirm', payload, signal, 'caller-signal-only'),
