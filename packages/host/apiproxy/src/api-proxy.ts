...
import { currentModelSelection, installModelSelection } from '@deepseek-ai/dsh-agent'
...
interface PendingApproval {
  rpcId: RpcId
  /** Exact live requester retained only until this question settles. */
  agent: Agent
  sessionId: SessionId
  ...
  callId?: CallId
  reason?: string
  /** Live TeamRun scope captured when the approval was published. */
  collaborationRunId?: SessionId
...
function requestedFrame(pending: PendingApproval): RpcRequest<MuxFrame> {
  return {
    ...
    payload: {
      ...
      ...pending.collaborationRunId === undefined
        ? {}
        : { collaborationRunId: pending.collaborationRunId },
    },
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  /** Process-local, fail-safe grants for one exact live collaboration task. */
  const collaborationTaskApprovalGrants = new Set<SessionId>()
  const muxQueues = new Set<FrameQueue<RpcRequest<MuxFrame>>>()
  const imageAdmissionChains = new WeakMap<Agent, Promise<void>>()

  /**
   * Resolve an agent to a currently executable TeamRun approval scope.
   * Draft, terminal, stale, failed-member, and ordinary conversation agents
   * deliberately have no task-wide scope.
   */
  function activeCollaborationApprovalRun(agent: Agent): SessionId | undefined {
    const teamRuns = ctx.get('teamRuns')
    if (teamRuns === undefined) return undefined
    const membership = teamRuns.tryMembership(agent)
    if (membership === undefined) return undefined
    try {
      const status = teamRuns.getRun(agent).status
      if (status !== 'running' && status !== 'blocked'
        && status !== 'reviewing' && status !== 'reworking') return undefined
      return brandSessionId(String(membership.runId))
    } catch {
      return undefined
    }
  }
  ...
  function selectionFor(agent: Agent): WebModelSelectionRef {
    ...
    const composed = currentModelSelection(agent.ctx)
    if (composed !== undefined) {
      if (composed.current === undefined) composed.current = defaults.defaultModelSelection()
      const adopted = composed as WebModelSelectionRef
      selections.set(agent, adopted)
      return adopted
    }
  ...
  if (ctx.get('approval') !== undefined) {
    ...
    ctx.on('approval/request', (req, next) => {
      ...
      const collaborationRunId = activeCollaborationApprovalRun(req.agent)
      // The ApprovalService already appended approval/asked before this
      // answerer runs and will append approval/decided after this return, so
      // task-wide reuse remains fully audited without publishing another UI
      // question.
      if (collaborationRunId !== undefined
        && collaborationTaskApprovalGrants.has(collaborationRunId)) {
      ...
      return new Promise<ApprovalOutcome>((resolve) => {
        ...
        const pending: PendingApproval = {
          ...
          ...collaborationRunId === undefined ? {} : { collaborationRunId },
          resolve: settle,
        }
  ...
  function collaborationWorkstream(
    value: TeamWorkstream & { readonly assigneeSlotId?: PlannedTeamWorkstream['assigneeSlotId'] },
  ): CollaborationWorkstreamView {
    return {
      id: value.id,
      ...
      ...value.assigneeSlotId === undefined ? {} : { assigneeSlotId: String(value.assigneeSlotId) },
    }
  }
  ...
  function collaborationBinding(
    ...
    plannedLocalSkills?: readonly string[],
    ...
    plannedModelSelection?: ModelSelection,
  ): CollaborationExpertBindingView {
    const descriptor = binding?.descriptor
    const preset = descriptor?.preset.id ?? blueprint.preset
    const skills = descriptor?.skills.map(value => value.name) ?? plannedLocalSkills ?? blueprint.skills
    const plugins = descriptor?.plugins ?? blueprint.plugins
    const runtimeFoundation = descriptor?.foundation
    const plannedMaxTokens = Math.min(
      blueprint.model.maxTokens ?? blueprint.budget.maxTokens,
      blueprint.budget.maxTokens,
    )
    const allow = [...blueprint.tools.allow ?? []]
    const deny = [...blueprint.tools.deny ?? []]
    const foundation: CollaborationExpertBindingView['foundation'] | undefined
      = binding !== undefined && runtimeFoundation === undefined
        ? undefined
        : runtimeFoundation === undefined
          ? {
            model: {
              mode: plannedModelSelection === undefined
                && blueprint.model.provider === undefined && blueprint.model.model === undefined
                ? 'inherit_lead'
                : 'selected',
              ...(plannedModelSelection?.provider ?? blueprint.model.provider) === undefined
                ? {}
                : { provider: plannedModelSelection?.provider ?? blueprint.model.provider },
              ...(plannedModelSelection?.model ?? blueprint.model.model) === undefined
                ? {}
                : { model: plannedModelSelection?.model ?? blueprint.model.model },
              ...plannedModelSelection?.reasoningEffort === undefined
                ? {}
                : { reasoningEffort: plannedModelSelection.reasoningEffort },
              maxTokens: plannedMaxTokens,
            },
            tools: {
              access: blueprint.tools.allow === undefined && deny.length === 0
                ? 'full_preset'
                : 'restricted',
              allow,
              deny,
            },
            permissions: { sandboxMode: 'inherit_lead', approvalPolicy: 'inherit_lead' },
          }
          : {
            model: {
              mode: 'selected',
              provider: runtimeFoundation.modelSelection.provider,
              model: runtimeFoundation.modelSelection.model,
              ...runtimeFoundation.modelSelection.reasoningEffort === undefined
                ? {}
                : { reasoningEffort: runtimeFoundation.modelSelection.reasoningEffort },
              maxTokens: descriptor?.model.maxTokens ?? plannedMaxTokens,
            },
            tools: { access: runtimeFoundation.toolAccess, allow, deny },
            permissions: { ...runtimeFoundation.permissions },
          }
    return {
      blueprint: {
  ...
  function collaborationView(lead: Agent, snapshot: TeamOrchestrationSnapshot): CollaborationRunView {
    ...
    const experts: CollaborationExpertView[] = (snapshot.plan?.roster ?? []).map((planned) => {
      const slotMembers = snapshot.run.members.filter(value =>
        value.protocolSlotId !== undefined && String(value.protocolSlotId) === String(planned.slotId))
      const member = slotMembers.find(value => value.phase === 'active')
        ?? slotMembers.at(-1)
        ?? snapshot.run.members.find(value => value.name === planned.name)
    ...
    return {
      ...
      charter,
      lead: {
        sessionId: brandSessionId(String(snapshot.run.lead.sessionId)),
        name: 'lead',
        role: 'Lead Agent',
        ...snapshot.profile.leadModelSelection === undefined
          ? {}
          : { modelSelection: { ...snapshot.profile.leadModelSelection } },
      },
      experts,
  ...
  /** Resolve live Leads plus the newest requested cold Leads with durable orchestration profiles. */
  async function collaborationLeads(limit?: number): Promise<Agent[]> {
    const found = new Map<SessionId, Agent>()
    for (const lead of ctx.agents.roots()) {
      if (lead.session.events.some(event => event.type === 'collaboration/orchestration/profile')) {
        found.set(lead.id, lead)
      }
    }
    const persistence = ctx.get('sessionPersistence')
    const candidates = persistence === undefined
      ? (await listVisibleSessionSummaries())
        .filter(summary => !found.has(summary.sessionId) && summary.parentSessionId === undefined)
        .map(summary => ({ sessionId: summary.sessionId, createdAt: summary.updatedAt }))
      : (await persistence.list())
        .filter(meta => !found.has(meta.id) && meta.parentSession === undefined && meta.cwd !== undefined)
        .map(meta => ({ sessionId: meta.id, createdAt: meta.createdAt }))
    ...
    for (let offset = 0; offset < candidates.length; offset += COLD_SUMMARY_BATCH_SIZE) {
      ...
      for (const sessionId of profileOwners) {
        ...
        if (!('error' in resolved)) {
          found.set(resolved.agent.id, resolved.agent)
          coldFound += 1
        }
      }
      if (limit !== undefined && coldFound >= limit) break
    }
    return [...found.values()]
  }
  ...
  return {
    ...
    collaboration: {
      async create(request, signal) {
        ...
        let leadModel: ModelSelection | undefined
        if (payload.leadModel !== undefined) {
          try {
            const resolved = await ctx.llm.resolveCallConfig({
              provider: payload.leadModel.provider,
              model: payload.leadModel.model,
              ...payload.leadModel.reasoningEffort === undefined
                ? {}
                : { reasoningEffort: ReasoningEffortId(payload.leadModel.reasoningEffort) },
            })
            leadModel = {
              provider: resolved.provider,
              model: resolved.model,
            ...
            return err(request, {
              code: 'model-unavailable',
              message: error instanceof Error ? error.message : String(error),
              details: { provider: payload.leadModel.provider, model: payload.leadModel.model },
            })
          }
        }
        let expertModels: CreateTeamOrchestrationRequest['expertModels']
        if (payload.expertModels !== undefined) {
          const validated: NonNullable<CreateTeamOrchestrationRequest['expertModels']>[number][] = []
          for (const entry of payload.expertModels) {
            try {
              const resolved = await ctx.llm.resolveCallConfig({
                provider: entry.selection.provider,
                model: entry.selection.model,
                ...entry.selection.reasoningEffort === undefined
                  ? {}
                  : { reasoningEffort: ReasoningEffortId(entry.selection.reasoningEffort) },
              })
              validated.push({
                slotId: entry.slotId,
                selection: {
              ...
              return err(request, {
                code: 'model-unavailable',
                message: error instanceof Error ? error.message : String(error),
                details: { provider: entry.selection.provider, model: entry.selection.model },
              })
            }
          }
          expertModels = validated
        }
        const orchestration: CreateTeamOrchestrationRequest = {
          ...
          ...expertModels === undefined ? {} : { expertModels },
          context: {
        ...
        try {
          signal.throwIfAborted()
          const planned = await collaborationOrchestrator().create(lead, orchestration)
          if (leadModel !== undefined) selectionFor(lead).current = leadModel
      ...
      async confirm(request, signal) {
        ...
        if ('error' in found) return err(request, found.error)
        try {
          const retained = collaborationOrchestrator().get(found.agent)
          if (retained.profile.leadModelSelection !== undefined) {
            selectionFor(found.agent).current = retained.profile.leadModelSelection
          }
          return ok(request, collaborationView(
            found.agent,
            await collaborationOrchestrator().form(found.agent, {
              requestId: TeamOrchestrationRequestId(request.payload.requestId),
            }, signal),
          ))
        } catch (error: unknown) {
          const terminal = terminalCollaborationView(found.agent)
          if (terminal !== undefined) return ok(request, terminal)
          if (signal.aborted) {
            return err(request, { code: 'cancelled', message: 'Team formation was cancelled.', details: {} })
          }
      ...
      async list(request) {
        try {
          const views: CollaborationRunView[] = []
          let firstFailure: unknown
          for (const lead of await collaborationLeads(request.payload.limit)) {
            try {
              views.push(collaborationView(lead, collaborationOrchestrator().get(lead)))
            } catch (error: unknown) {
              firstFailure ??= error
              ctx.logger.warn(`collaboration run "${lead.id}" was omitted from the catalog because replay failed: ${error instanceof Error ? error.message : String(error)}`)
            }
          }
          if (views.length === 0 && firstFailure !== undefined) {
            throw firstFailure instanceof Error
              ? firstFailure
              : new Error('collaboration run replay failed with a non-Error value')
          }
          views.sort((left, right) => right.createdAt - left.createdAt)
          return ok(request, { runs: request.payload.limit === undefined ? views : views.slice(0, request.payload.limit) })
        } catch (error: unknown) {
    ...
    respond(message: ClientResponse): Promise<RpcReceipt> {
      ...
      if (approval !== undefined) {
        ...
        if (parsed.data.outcome === 'allowed-for-task') {
          const runId = approval.collaborationRunId
          if (runId === undefined) return Promise.resolve({ accepted: false, reason: 'bad-response' })
          if (activeCollaborationApprovalRun(approval.agent) !== runId) {
            return Promise.resolve({ accepted: false, reason: 'not-pending' })
          }
          collaborationTaskApprovalGrants.add(runId)
          // Parallel experts may already be waiting. Settle every question in
          // this TeamRun now; a request dispatched after the grant observes
          // the fast path above, leaving no approval gap between the two.
          for (const candidate of [...pendingApprovals.values()]) {
            if (candidate.collaborationRunId === runId) candidate.resolve('allowed-once')
          }
