...
describe('events frame schemas', () => {
  it('accepts every mux frame branch', () => {
    const frames = [
      ...
      { type: 'approval/requested', sessionId: 's', approvalId: 'a', toolName: 'bash', callId: 'c', reason: 'r', collaborationRunId: 'run-1' },
...
describe('respond payload schemas', () => {
  it('validates approval and question answer payloads', () => {
    ...
    const approval = approvalResponsePayloadSchema.parse({ sessionId: 's', approvalId: 'a', outcome: 'allowed-for-task' })
    expect(approval.outcome).toBe('allowed-for-task')
