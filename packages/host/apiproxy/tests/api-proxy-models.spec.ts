...
describe('Web session model selection', () => {
  it('adopts an expert runtime model selection so the daily model API mutates the same session route', async () => {
    ...
    expect(expectValue(await api.sessions.models(request({ sessionId }))).current).toEqual(selection.current)
