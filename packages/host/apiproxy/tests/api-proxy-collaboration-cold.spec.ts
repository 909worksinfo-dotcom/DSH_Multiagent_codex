...
describe('collaboration cold restart', () => {
      ...
      value: {
        runs: [{
          ...
          experts: [{
            binding: {
              foundation: {
                model: { mode: 'inherit_lead', maxTokens: 1_024 },
  ...
  it('keeps healthy collaboration runs visible when another historical run is incompatible', async () => {
    const ctx = new Context()
    contexts.add(ctx)
    await ctx.plugin(SessionStore)
    await ctx.plugin(AgentRegistry)
    await ctx.plugin(UserQuestionService)
    const healthyId = SessionId('collaboration-healthy-live')
    const incompatibleId = SessionId('collaboration-incompatible-live')
    const healthy = collaborationFixture(healthyId)
    const incompatible = collaborationFixture(incompatibleId)
    for (const [id, fixture] of [[healthyId, healthy], [incompatibleId, incompatible]] as const) {
      const session = { id, header: header(id), events: fixture.events } as unknown as Session
      ctx.agents.register({ id, session, status: 'idle', ctx } as Agent)
    }
    ctx.provide('teamOrchestrator', {
      list: () => { throw new Error('one incompatible live root must not poison discovery') },
      get: (lead: Agent) => {
        if (lead.id === incompatibleId) throw new Error('historical orchestration replay is incompatible')
        return healthy.snapshot
      },
    } as never)
    ctx.provide('expertCatalog', { get: () => healthy.blueprint } as never)
    ctx.provide('sessionPersistence', {
      list: () => Promise.resolve([]),
      inspect: () => Promise.reject(new Error('not expected')),
      locate: () => undefined,
    } as never)
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'test', model: 'test' }),
    ...
    expect(response.result).toMatchObject({
      ok: true,
      value: { runs: [{ id: healthyId, title: 'Restart-safe research' }] },
    })
  })

  it('honors a bounded newest-first collaboration catalog request', async () => {
    const ctx = new Context()
    contexts.add(ctx)
    await ctx.plugin(SessionStore)
    await ctx.plugin(AgentRegistry)
    await ctx.plugin(UserQuestionService)
    const olderId = SessionId('collaboration-older-live')
    const newerId = SessionId('collaboration-newer-live')
    const older = collaborationFixture(olderId)
    const newer = collaborationFixture(newerId)
    for (const [id, fixture] of [[olderId, older], [newerId, newer]] as const) {
      const session = { id, header: header(id), events: fixture.events } as unknown as Session
      ctx.agents.register({ id, session, status: 'idle', ctx } as Agent)
    }
    ctx.provide('teamOrchestrator', {
      list: () => [],
      get: (lead: Agent) => lead.id === newerId
        ? { ...newer.snapshot, createdAt: 2_000 }
        : { ...older.snapshot, createdAt: 1_000 },
    } as never)
    ctx.provide('expertCatalog', { get: () => newer.blueprint } as never)
    ctx.provide('sessionPersistence', {
      list: () => Promise.resolve([]),
      inspect: () => Promise.reject(new Error('not expected')),
      locate: () => undefined,
    ...
    const response = await api.collaboration.list(request({ limit: 1 }))

    expect(response.result).toMatchObject({
      ok: true,
      value: { runs: [{ id: newerId }] },
    })
    if (response.result.ok) expect(response.result.value.runs).toHaveLength(1)
  })

  it('confirms a research plan through the inherited agent foundation without a collaboration credential preflight', async () => {
    const ctx = new Context()
    contexts.add(ctx)
    await ctx.plugin(SessionStore)
    await ctx.plugin(AgentRegistry)
    await ctx.plugin(UserQuestionService)
    const id = SessionId('collaboration-research-credential-preflight')
    const fixture = collaborationFixture(id)
    const session = { id, header: header(id), events: fixture.events } as unknown as Session
    const lead = { id, session, status: 'idle', ctx } as Agent
    ...
    const form = vi.fn(async () => fixture.snapshot)
    ctx.provide('teamOrchestrator', { get: () => planned, form } as never)
    ctx.provide('expertCatalog', { get: () => fixture.blueprint } as never)
    const api = createApiProxy(ctx, {
      defaultModelSelection: () => ({ provider: 'test', model: 'test' }),
      cwd: '/project',
    })

    const response = await api.collaboration.confirm(request({
      runId: id,
      requestId: String(fixture.snapshot.requestId),
    }), new AbortController().signal)

    expect(response.result).toMatchObject({
      ok: true,
      value: { id, phase: fixture.snapshot.run.phase },
    })
