...
describe('unary round trip', () => {
  ...
  it('round-trips a collaboration run while stripping private composition metadata', async () => {
    ...
    const response = await client(api).collaboration.create({
      ...
      language: 'zh',
      domain: 'research_analysis',
      leadModel: { provider: 'deepseek-official', model: 'deepseek-v4-flash' },
      expertModels: [{
        slotId: 'slot-1',
  ...
  it('routes an exact reviewed-plan confirmation through the typed carrier', async () => {
    let seen: RpcRequest<unknown> | undefined
    const run = collaborationRun()
    const api = scriptedApi({
      collaboration: {
    ...
    const response = await client(api).collaboration.confirm({
      runId: sid('lead-1'),
      requestId: 'request-1',
    })

    expect(seen?.payload).toEqual({ runId: 'lead-1', requestId: 'request-1' })
    expect(response.result).toMatchObject({ ok: true, value: { id: run.id, phase: run.phase } })
  })
