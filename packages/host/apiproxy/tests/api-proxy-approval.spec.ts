...
async function harness(options: {
  readonly collaborationRunOf?: (agent: Agent) => string | undefined
  ...
  if (options.collaborationRunOf !== undefined) {
    ctx.provide('teamRuns', {
      tryMembership: (agent: Agent) => {
        const runId = options.collaborationRunOf?.(agent)
        return runId === undefined ? undefined : { runId, actor: { role: 'lead', sessionId: runId, name: 'lead' } }
      },
...
function answer(rpcId: RpcId, sessionId: unknown, approvalId: ApprovalRequestId, outcome: 'allowed-once' | 'allowed-for-turn' | 'allowed-for-task' | 'rejected'): Parameters<ApiProxy['respond']>[0] {
...
describe('approval pending registry', () => {
  ...
  it('grants every pending and future approval in one collaboration task without affecting another session', async () => {
    const runId = 'collaboration-run-1'
    const runByAgent = new WeakMap<Agent, string>()
    const { ctx, api } = await harness({ collaborationRunOf: agent => runByAgent.get(agent) })
    const abort = new AbortController()
    const mux = openMux(api, abort)
    const firstExpert = agentOf(ctx)
    const secondExpert = agentOf(ctx)
    const unrelated = agentOf(ctx)
    runByAgent.set(firstExpert, runId)
    runByAgent.set(secondExpert, runId)

    const firstAsk = ctx.approval.request({ agent: firstExpert, toolName: 'bash', reason: 'first expert' })
    const secondAsk = ctx.approval.request({ agent: secondExpert, toolName: 'write', reason: 'second expert' })
    await waitForCount(mux, 'approval/requested', 2)
    const pending = mux.envelopes.filter(envelope => envelope.payload.type === 'approval/requested')
    expect(pending.map(envelope => requestedOf(envelope.payload).collaborationRunId))
      .toEqual([runId, runId])

    const first = pending.find(envelope => requestedOf(envelope.payload).toolName === 'bash') as RpcRequest<MuxFrame>
    const firstRequested = requestedOf(first.payload)
    expect(await api.respond(answer(
      first.rpcId,
      firstRequested.sessionId,
      firstRequested.approvalId,
      'allowed-for-task',
    ))).toEqual({ accepted: true })
    await expect(firstAsk).resolves.toBe('allowed-once')
    await expect(secondAsk).resolves.toBe('allowed-once')
    await waitForCount(mux, 'approval/resolved', 2)

    const requestedBeforeReuse = mux.frames.filter(frame => frame.type === 'approval/requested').length
    await expect(ctx.approval.request({ agent: secondExpert, toolName: 'network', reason: 'future expert call' }))
      .resolves.toBe('allowed-once')
    expect(mux.frames.filter(frame => frame.type === 'approval/requested')).toHaveLength(requestedBeforeReuse)

    const unrelatedAsk = ctx.approval.request({ agent: unrelated, toolName: 'bash', reason: 'daily session' })
    await waitForCount(mux, 'approval/requested', requestedBeforeReuse + 1)
    const unrelatedEnvelope = mux.envelopes.find(envelope =>
      envelope.payload.type === 'approval/requested'
      && envelope.payload.sessionId === unrelated.session.id) as RpcRequest<MuxFrame>
    const unrelatedRequested = requestedOf(unrelatedEnvelope.payload)
    expect(unrelatedRequested.collaborationRunId).toBeUndefined()
    expect(await api.respond(answer(
      unrelatedEnvelope.rpcId,
      unrelatedRequested.sessionId,
      unrelatedRequested.approvalId,
      'rejected',
    ))).toEqual({ accepted: true })
    await expect(unrelatedAsk).resolves.toBe('rejected')
    abort.abort()
  })
