...
`collaboration.create` 接受可选的主协调智能体模型选择与专家席位选择，并在编排器持久化 profile 和 plan 前，使用与 `session.selectModel` 相同的 LLM adapter 校验解析每条路由。确认执行会在组队前重新安装已保留的主协调智能体路由，包括冷恢复场景。计划视图只公开选中的 Provider、模型、思考强度和 Token 上限，不可用路由会在创建专家前失败

协作审批增加仅供 Web 使用的 `allowed-for-task` 应答，但不扩大核心审批结果词汇。只有处于运行中 TeamRun 的成员，其审批请求帧才会携带 `collaborationRunId`。用户接受任务级授权后，网关会为这个精确 TeamRun 安装进程内授权，把所有已经等待的成员审批以有审计记录的一次性授权统一放行，并让之后任意活跃成员的审批无需再次弹窗。日常会话、失效成员、终态任务与进程重启都不会继承该授权
