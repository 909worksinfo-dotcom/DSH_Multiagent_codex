...
describe('ExpertCatalog', () => {
  ...
  it('resolves a task-reviewed skill set without mutating the catalog blueprint', async () => {
    const configured = blueprint()
    const { catalog, getSkill } = await setup({ blueprints: [configured] })
    const defaults = await catalog.resolve(configured.ref)
    getSkill.mockClear()

    const reviewed = await catalog.resolve(configured.ref, {
      skills: ['technical-review', 'risk-analysis'],
    })

    expect(reviewed.blueprint.skills).toEqual(['technical-review', 'risk-analysis'])
    expect(reviewed.skills.map(value => value.name)).toEqual(['technical-review', 'risk-analysis'])
    expect(reviewed.digest).not.toBe(defaults.digest)
    expect(catalog.get(configured.ref).skills).toEqual(['literature-search'])
    expect(getSkill.mock.calls.map((call): unknown => call[0])).toEqual(['technical-review', 'risk-analysis'])
  })
