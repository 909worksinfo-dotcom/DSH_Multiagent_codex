...
import { z } from 'zod';
import { ExpertBlueprintId } from "./ids.js";
const id = z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/);
const text = (maximum) => z.string().trim().min(1).max(maximum);
const uniqueStrings = (label, maximum) => z.array(text(maximum)).max(128).superRefine((values, ctx) => {
    const seen = new Set();
    for (const [index, value] of values.entries()) {
        if (seen.has(value))
            ctx.addIssue({ code: 'custom', message: `duplicate ${label} ${JSON.stringify(value)}`, path: [index] });
        seen.add(value);
    }
});
const fields = z.array(z.object({
    ...
    for (const [index, value] of values.entries()) {
        if (seen.has(value.name))
            ctx.addIssue({ code: 'custom', message: `duplicate field ${JSON.stringify(value.name)}`, path: [index, 'name'] });
        seen.add(value.name);
    }
});
const toolPolicy = z.object({
    ...
    for (const denied of value.deny ?? []) {
        if (allowed.has(denied))
            ctx.addIssue({ code: 'custom', message: `tool ${JSON.stringify(denied)} cannot be both allowed and denied` });
    }
});
 ...
 */
export function parseBlueprint(value) {
    return blueprint.parse(value);
}
