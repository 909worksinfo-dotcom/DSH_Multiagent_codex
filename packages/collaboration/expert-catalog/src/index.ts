...
export class ExpertCatalog extends Service {
   ...
   * @param ref - exact immutable selector.
   * @param options - cwd-sensitive lookup, optional task-reviewed skill set, and caller cancellation.
  ...
  async resolve(ref: ExpertBlueprintRef, options: ResolveExpertBindingOptions = {}): Promise<ResolvedExpertBinding> {
    const configured = this.get(ref)
    const requestedSkills = options.skills?.map(value => value.trim())
    if (requestedSkills !== undefined
      && (requestedSkills.length === 0
        || requestedSkills.length > 200
        || requestedSkills.some(value => value === '')
        || new Set(requestedSkills).size !== requestedSkills.length)) {
      throw new TeamRunError('task-reviewed expert skills must contain 1 to 200 distinct non-blank names', 'TEAM_INVALID_ARGUMENT')
    }
    const blueprint: ExpertBlueprint = requestedSkills === undefined
      ? configured
      : { ...configured, skills: requestedSkills }
