...
async function setup(options: {
  ...
  const getBlueprint = vi.fn((ref: ExpertBlueprint['ref']) => {
  ...
  const resolveBlueprint = vi.fn(async (
    ref: ExpertBlueprint['ref'],
    resolveOptions: { readonly skills?: readonly string[] } = {},
  ) => {
    const value = getBlueprint(ref)
    const skills = resolveOptions.skills ?? value.skills
    const available = new Set([...value.skills, 'demo-software-skill'])
    const missing = skills.find(skill => !available.has(skill))
    if (missing !== undefined) throw new TeamRunError(`missing skill "${missing}"`, 'CAPABILITY_UNAVAILABLE')
    return { blueprint: { ...value, skills } } as never
  })
  ctx.provide('expertCatalog', { get: getBlueprint, resolve: resolveBlueprint } as never)
  ...
  const marketplacePrepare = vi.fn(async (capability: SkillMarketplaceCapability) => capability)
  const marketplaceSearch = vi.fn<(query: string) => Promise<SkillMarketplaceSearchResult>>(async () => ({
    query: 'demo expert',
    providers: [
      {
      ...
      { source: 'composio' as const, state: 'authorization_required' as const, capabilities: [] },
    ],
  }))
  if (options.discoverMarketplaceSkills === true) {
    ctx.provide('skillMarketplace', {
      prepare: marketplacePrepare,
      search: marketplaceSearch,
    } as never)
  }
  const fiber = await ctx.plugin(TeamOrchestrator, config(options.count))
  return {
    ctx, lead, fiber, provision, recoverProvisioning, getBlueprint,
    marketplacePrepare, marketplaceSearch,
  }
}
...
describe('TeamOrchestrator lifecycle', () => {
  it('fails closed before provisioning when any planned expert declares fewer than two skills', async () => {
    const { ctx, lead, provision, getBlueprint } = await setup()
    getBlueprint.mockImplementation((ref) => {
      const resolved = blueprint(Number(String(ref.id).split('-').at(-1)))
  ...
  it('does not publish a reviewable plan when an explicit per-expert skill floor is unmet', async () => {
    const { ctx, lead, provision } = await setup()

    await expect(ctx.teamOrchestrator.create(lead, {
      requestId: TeamOrchestrationRequestId('reviewed-skill-floor'),
      objective: '分析一项并购\n\n协作方案调整要求：\n每个专家挂载3个技能',
    })).rejects.toMatchObject({ code: 'CAPABILITY_UNAVAILABLE' })

    expect(ctx.teamOrchestrator.get(lead).run.phase).toBe('formation_failed')
    expect(provision).not.toHaveBeenCalled()
    expect(lead.session.events.filter(event => event.type === 'collaboration/orchestration/plan')).toHaveLength(0)
  })

  it('does not publish a reviewable plan when a requested expert skill cannot resolve', async () => {
    const { ctx, lead, provision } = await setup()

    await expect(ctx.teamOrchestrator.create(lead, {
      requestId: TeamOrchestrationRequestId('unavailable-reviewed-skill'),
      objective: [
        'Analyze an acquisition and review the evidence',
        '',
        '协作方案调整要求：',
        '将 demo-expert-1 的技能修改为 demo-professional-skill、missing-requested-skill',
      ].join('\n'),
    })).rejects.toMatchObject({ code: 'CAPABILITY_UNAVAILABLE' })
  ...
  it('keeps the exact charter and roster unprovisioned until the reviewed plan is confirmed', async () => {
    const { ctx, lead, provision } = await setup()
    const requestId = TeamOrchestrationRequestId('review-before-formation')

    const planned = await ctx.teamOrchestrator.create(lead, {
      requestId,
      objective: 'Summarize the evidence',
    })

    expect(planned.run).toMatchObject({ phase: 'planning', status: 'forming', plannedExperts: 3 })
    expect(planned.plan?.roster).toHaveLength(3)
    expect(planned.charter?.taskDag.every(task => task.assigneeSlotId !== undefined)).toBe(true)
    expect(planned.run.members).toEqual([])
    expect(planned.run.tasks).toEqual([])
    expect(provision).not.toHaveBeenCalled()

    const confirmed = await ctx.teamOrchestrator.form(lead, { requestId }, new AbortController().signal)
    expect(confirmed.run).toMatchObject({ phase: 'active', status: 'running' })
    expect(confirmed.run.members).toHaveLength(3)
    expect(confirmed.run.tasks).toHaveLength(planned.charter?.taskDag.length ?? 0)
    expect(provision).toHaveBeenCalledTimes(3)
  })

  it('passes a reviewed per-expert local-skill replacement into real provisioning', async () => {
    const { ctx, lead, provision } = await setup()
    const requestId = TeamOrchestrationRequestId('reviewed-local-skill-provisioning')
    const localSkills = ['demo-professional-skill', 'demo-software-skill']
    const planned = await ctx.teamOrchestrator.create(lead, {
      requestId,
      objective: [
        'Analyze an acquisition and review the evidence',
        '',
        '协作方案调整要求：',
        `将 demo-expert-1 的技能修改为 ${localSkills.join('、')}`,
      ].join('\n'),
    })

    expect(planned.plan?.roster.find(expert => String(expert.blueprint.id) === 'demo-expert-1')?.localSkills)
      .toEqual(localSkills)
    await ctx.teamOrchestrator.form(lead, { requestId }, new AbortController().signal)
    expect(provision).toHaveBeenCalledWith(
      lead,
      expect.objectContaining({ localSkills }),
    )
  })

  it('persists selected market skills in the plan and passes the exact mounts to expert provisioning', async () => {
    const { ctx, lead, provision, marketplaceSearch } = await setup({ discoverMarketplaceSkills: true })
    const formed = await ctx.teamOrchestrator.orchestrate(lead, {
      requestId: TeamOrchestrationRequestId('marketplace-mounts'),
      objective: 'Summarize the evidence',
    }, new AbortController().signal)
    const expectedMounts = [expect.objectContaining({
      id: 'skills.sh:trusted/research-method',
      name: 'Research method',
      status: 'loaded',
    })]
    ...
    expect(marketplaceSearch).toHaveBeenCalledWith(expect.stringMatching(/^Demo expert .*Summarize evidence$/u))
    for (const call of provision.mock.calls) {
      expect(call[1]).toMatchObject({ marketplaceSkills: expectedMounts })
      expect(call[1].marketplaceSkills).not.toEqual(expect.arrayContaining([
        expect.objectContaining({ status: 'authorization_required' }),
      ]))
    }
  })

  it('uses a bounded provider-friendly query for a long multilingual task', async () => {
    const { ctx, lead, marketplaceSearch } = await setup({ discoverMarketplaceSkills: true })
    ...
    const queries = marketplaceSearch.mock.calls.map(call => call[0])
  ...
  it('prepares only the bounded selected mounts and promotes their provider to ready', async () => {
    const { ctx, lead, provision, marketplacePrepare } = await setup({ discoverMarketplaceSkills: true })
    marketplacePrepare.mockImplementation(async capability => capability.source === 'smithery'
      ? {
        ...capability,
        access: 'public',
        status: 'connected',
    ...
    const formed = await ctx.teamOrchestrator.orchestrate(lead, {
      ...
      objective: 'Use the Remote search integration to summarize evidence',
    }, new AbortController().signal)

    expect(marketplacePrepare).toHaveBeenCalledTimes(4)
    expect(formed.plan?.roster.filter(expert => expert.skillDiscovery?.providers
      .some(provider => provider.source === 'smithery' && provider.state === 'ready'))).toHaveLength(1)
    expect(formed.plan?.roster.filter(expert => expert.skillDiscovery?.mounts
      .some(capability => capability.id === 'smithery:trusted/search'
        && capability.status === 'connected' && capability.access === 'public'))).toHaveLength(1)
    expect(provision.mock.calls.filter(call => call[1].marketplaceSkills
      ?.some(capability => capability.status === 'connected'))).toHaveLength(1)
  })

  it('keeps research methods without treating a topic-only entity mention as a remote-tool request', async () => {
    const { ctx, lead, provision, getBlueprint, marketplacePrepare, marketplaceSearch } = await setup({ discoverMarketplaceSkills: true })
    const specialists = [
    ...
    const formed = await ctx.teamOrchestrator.orchestrate(lead, {
      ...
      objective: '请分析英伟达以超 130 亿美元收购 Hugging Face 的战略利弊。利端：整合开源模型生态、强化 AI 开发者粘性、完善“硬件 + 平台”闭环、获取数据与社区资产。请分点论述并给出综合判断，并生成飞书文档',
    }, new AbortController().signal)
    ...
    expect(discoveries.filter(names => names?.includes('Hugging Face')).length).toBe(0)
    ...
    expect(formed.plan?.roster.find(expert => String(expert.blueprint.id) === 'demo-expert-3')
      ?.skillDiscovery?.mounts.map(value => value.name)).not.toContain('Hugging Face')
    expect(formed.plan?.roster.filter(expert => expert.skillDiscovery?.providers
      .some(provider => provider.source === 'smithery' && provider.state === 'authorization_required'))).toHaveLength(0)
    expect(formed.plan?.roster.filter(expert => expert.skillDiscovery?.providers
      .some(provider => provider.source === 'smithery' && provider.state === 'unavailable'))).toHaveLength(3)
    expect(marketplacePrepare).toHaveBeenCalledTimes(3)
    expect(marketplacePrepare.mock.calls.every(call => call[0].kind === 'method_skill')).toBe(true)
    expect(provision.mock.calls.filter(call => call[1].marketplaceSkills
      ?.some(capability => capability.name === 'Hugging Face')).length).toBe(0)
  })

  it('selects an authorization-gated remote tool only when the task explicitly requests it', async () => {
    const { ctx, lead, marketplacePrepare } = await setup({ discoverMarketplaceSkills: true })
    const planned = await ctx.teamOrchestrator.create(lead, {
      requestId: TeamOrchestrationRequestId('explicit-remote-tool-intent'),
      objective: '请使用 Remote search API 查询外部数据并总结',
    })

    expect(planned.plan?.roster.filter(expert => expert.skillDiscovery?.providers
      .some(provider => provider.source === 'smithery' && provider.state === 'authorization_required'))).toHaveLength(1)
    expect(planned.plan?.roster.filter(expert => expert.skillDiscovery?.providers
      .some(provider => provider.source === 'smithery' && provider.state === 'unavailable'))).toHaveLength(2)
    expect(marketplacePrepare).toHaveBeenCalledTimes(4)
    expect(marketplacePrepare.mock.calls.filter(call => call[0].id === 'smithery:trusted/search')).toHaveLength(1)
  })
