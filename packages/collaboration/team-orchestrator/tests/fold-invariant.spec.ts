...
describe('TeamOrchestrator replay and invariant companion', () => {
  ...
  it('replays a reviewed serial-stage plan against the same canonical DAG used by planning', () => {
    const runId = TeamRunId('lead-reviewed-fold')
    const requestId = TeamOrchestrationRequestId('request-reviewed-fold')
    const reviewedProfile: TaskProfile = {
      ...profile(),
      objective: '分析并评审证据\n\n协作方案调整要求：\n阶段1两个任务变成串行任务',
      workstreamSource: 'explicit',
      workstreams: [
        { id: 'collect-evidence', subject: '收集证据', description: '收集证据', blockedBy: [], requiredCapabilities: [], resourceScopes: [] },
        { id: 'review-evidence', subject: '评审证据', description: '评审证据', blockedBy: [], requiredCapabilities: [], resourceScopes: [] },
      ],
      planRequirements: { stageModes: [{ order: 1, mode: 'serial' }] },
      metrics: {
        ...profile().metrics,
        workstreamCount: 2,
    ...
    const requestDigest = digestJson({ requestId, retryOf: null, profile: reviewedProfile })
    const profileEvent = {
      type: 'collaboration/orchestration/profile',
      data: {
        version: 1,
        eventId: TeamOrchestrationEventId('reviewed-profile-event'),
        runId,
        requestId,
        requestDigest,
        revision: 1,
    ...
    const plan: TeamPlan = {
      ...
      roster: [{
        ...
        name: 'expert-1',
        role: '证据评审专家',
        blueprint: { id: ExpertBlueprintId('reviewer'), revision: 1 },
        assignment: { objective: reviewedProfile.objective, inputs: {} },
        acceptanceCriteria: ['完成评审'],
        budget: { maxTurns: 2, maxTokens: 1_024, timeoutMs: 60_000 },
      }],
      taskDag: [
        { ...reviewedProfile.workstreams[0]!, assigneeSlotId: TeamPlanSlotId('slot-1') },
        { ...reviewedProfile.workstreams[1]!, blockedBy: ['collect-evidence'], assigneeSlotId: TeamPlanSlotId('slot-1') },
      ],
      stages: [
        { id: 'stage-1', order: 1, mode: 'serial', workstreamIds: ['collect-evidence'] },
        { id: 'stage-2', order: 2, mode: 'serial', workstreamIds: ['review-evidence'] },
      ],
    }
    const planEvent = {
      type: 'collaboration/orchestration/plan',
      data: {
        version: 1,
        eventId: TeamOrchestrationEventId('reviewed-plan-event'),
        runId,
        requestId,
        requestDigest,
        revision: 2,
    ...
    expect(foldTeamOrchestration(runId, [profileEvent, planEvent]).plan?.plan.stages).toEqual(plan.stages)
  })
