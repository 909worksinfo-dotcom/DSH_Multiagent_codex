...
export function profileTask(request: CreateTeamOrchestrationRequest, config: Config): TaskProfile {
  const objective = assertText(request.objective, 'objective', config.maxTextBytes)
  ...
  const workstreams = normalizeWorkstreams({ ...request, objective }, config, signalText, language)
