...
export interface CreateTeamOrchestrationRequest {
  ...
  /** Review-time model route selected for the Lead coordinator. */
  readonly leadModel?: ModelSelection
  /** Review-time model routes selected for exact stable roster slots. */
  readonly expertModels?: readonly {
...
/** User-authored structural constraints that the generated plan must satisfy. */
export interface TeamPlanRequirements {
  /** Minimum count of executable local or ready task-bound skills mounted per expert. */
  readonly minimumSkillsPerExpert?: number
  /** Ordered user-requested mutations to one expert or the complete planned roster. */
  readonly expertSkills?: readonly {
    readonly target: string
    readonly operation: 'replace' | 'add' | 'remove'
    readonly skills: readonly string[]
  }[]
  /** Requested execution mode for the dependency stage visible before the adjustment is applied. */
  readonly stageModes?: readonly {
    readonly order: number
    readonly mode: 'serial' | 'parallel'
  }[]
  /** User-visible instruction appended to one exact one-based task in its original dependency stage. */
  readonly taskInstructions?: readonly {
    readonly stageOrder?: number
    readonly taskOrder: number
    readonly instruction: string
  }[]
  /** Exact model route selected for one stable roster slot before formation. */
  readonly expertModels?: readonly {
...
export interface TaskProfile {
  ...
  /** Exact reviewed model route for the Lead coordinator. */
  readonly leadModelSelection?: ModelSelection
  /** Parsed review requirements retained for deterministic plan enforcement. */
  readonly planRequirements?: TeamPlanRequirements
...
export interface PlannedExpert {
  ...
  /** User-reviewed route overriding both the blueprint and Lead defaults. */
  readonly modelSelection?: ModelSelection
  /** Exact task-reviewed local skill set; absent only on legacy plans. */
