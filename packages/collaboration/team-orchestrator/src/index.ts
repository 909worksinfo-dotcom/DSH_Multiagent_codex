...
export class TeamOrchestrator extends Service {
    ...
    return this.locks.run(lead.id, async () => {
      ...
      try {
        ...
        for (const [index, planned] of state.plan.plan.roster.entries()) {
          formationSignal.throwIfAborted()
          this.requireMinimumExpertSkills(planned.blueprint)
  ...
  private requireMinimumExpertSkills(blueprintRef: ExpertBlueprintRef): ExpertBlueprint {
    const blueprint = this.ctx.expertCatalog.get(blueprintRef)
    const skillCount = new Set(blueprint.skills).size
  ...
  private selectTeamExpertSkillMounts(
    ...
    const entityOccurrences = new Map<string, { readonly index: number; readonly capability: SkillMarketplaceCapability }[]>()
    searches.forEach((search, index) => {
      for (const capability of search.candidates) {
        if (capability.kind !== 'remote_tool' || !capabilityMatchesTaskEntity(capability, entityTerms)) continue
        const occurrences = entityOccurrences.get(capability.id) ?? []
        occurrences.push({ index, capability })
        entityOccurrences.set(capability.id, occurrences)
      }
    })
    ...
    // With no named task entity, retain the normal role-query behavior for general remote tools.
    if (entityOccurrences.size === 0) {
      searches.forEach((search, index) => {
        for (const capability of search.candidates) {
          if (capability.kind !== 'remote_tool') continue
          add(index, capability)
        }
      })
    }
  ...
  private async prepareExpertSkillDiscovery(
    ...
    const preparedMounts = await Promise.all(mounts.map(async (capability) => {
      try {
    ...
    return {
      providers: search.providers.map(({ source }) => {
        const selected = preparedMounts.filter(capability => capability.source === source)
