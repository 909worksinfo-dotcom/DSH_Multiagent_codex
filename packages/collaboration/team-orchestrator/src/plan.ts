...
function inferredTaskDag(profile: TaskProfile, language: 'zh' | 'en'): TeamWorkstream[] {
  const description = profile.objective
...
export function planTeam(catalog: PlannerCatalog, config: Config, profile: TaskProfile): CurrentTeamPlan {
  ...
  const roster = selected.map(({ blueprint }, index): PlannedExpert => {
  ...
  const taskDag = assignTaskDag(executionTaskDag(profile), roster, selected.map(value => value.blueprint))
