...
调用方可为任意稳定 `slot-N` 团队席位指定经过校验的 Provider、模型与可选思考强度。该选择会保存在不可变 plan 中，冷恢复后仍保持一致，并在对应专家首轮执行前传给 ExpertRuntime

调用方还可在 Task Profile 中保留一条已审阅的主协调智能体路由。Host 会在组队前重新安装该路由，因此协调者与每名专家可以使用彼此独立的模型选择
