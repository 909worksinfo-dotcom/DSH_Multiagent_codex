...
Callers may attach an exact validated provider, model, and optional reasoning effort to any stable `slot-N` roster seat. The selection is retained in the immutable plan, survives cold replay, and is passed to ExpertRuntime before that expert's first turn

Callers may also retain one reviewed Lead route in the Task Profile. The Host reinstalls that route before formation so the coordinator and every expert can use independent model selections
