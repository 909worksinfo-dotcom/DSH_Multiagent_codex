...
An optional reviewed `modelSelection` overrides both blueprint and Lead routes. Its Provider, model, and reasoning effort are recorded in the immutable foundation and installed before initial prompt admission, while the blueprint budget continues to enforce the output-token ceiling
