...
/** Recover one durable incomplete turn that may have happened before this runtime observed the child. */
function latestExecutionFailure(session: Pick<Session, 'events'>): TeamRunError | undefined {
  ...
  switch (reason.kind) {
    ...
    case 'error':
      ...
      upstreamCode = reason.error.code
      break
    case 'interrupted':
      message = 'process stopped before the expert turn completed'
      break
    case 'max-tokens':
      message = 'expert turn reached its output token limit before completion'
      break
    case 'blocked':
      message = 'expert turn was blocked before execution completed'
      break
    case 'aborted':
      message = `expert turn was cancelled before completion (${reason.reason.kind})`
      break
  }
  return new TeamRunError(
    `expert latest execution failed: ${message}`,
...
function compositionDigest(
  ...
  foundation?: ExpertBindingDescriptor['foundation'],
): string {
...
export class ExpertRuntime extends Service {
  ...
  private readonly approvalWaits = new Map<SessionId, {
  ...
  constructor(ctx: Context, readonly config: Config) {
    ...
    ctx.on('agent/status', ({ agent, status }) => {
      ...
      if ((this.approvalWaits.get(agent.id)?.ids.size ?? 0) > 0) return
    ...
    ctx.on('agent/error', ({ agent, error }) => {
      const descriptor = foldExpertChildDescriptor(agent.session)
      if (descriptor === undefined || this.parentRunIsTerminal(agent)) return
      void this.failFromChild(agent, descriptor, error).catch((failureError: unknown) => {
        this.ctx.logger.warn(`expert "${agent.id}" execution failure record was rejected: ${errorMessage(failureError)}`)
      })
    })
    const reconcileRestoredLead = (agent: Agent): void => {
      queueMicrotask(() => {
        void this.reconcilePersistedFailures(agent).catch((error: unknown) => {
          this.ctx.logger.warn(`restored TeamRun expert failure reconciliation was deferred: ${errorMessage(error)}`)
        })
      })
    }
    ctx.on('agent/created', ({ agent }) => { reconcileRestoredLead(agent) })
    for (const agent of ctx.agents.list()) reconcileRestoredLead(agent)
    ctx.on('session/event', (session, event) => {
      if (event.type !== 'approval/asked' && event.type !== 'approval/decided') return
      const agent = ctx.agents.get(session.id)
      if (agent === undefined) return
      const descriptor = foldExpertChildDescriptor(session)
      if (descriptor === undefined) return
      if (event.type === 'approval/asked') {
        this.pauseForApproval(agent.id, String(event.data.id))
        return
      }
      this.resumeAfterApproval(agent, descriptor, String(event.data.id))
    })
    ...
    ctx.on('agent/pre-step', async ({ agent, signal }, next): Promise<PreStepDecision> => {
      ...
      try {
        ...
        if (!this.deadlines.has(agent.id) && (this.approvalWaits.get(agent.id)?.ids.size ?? 0) === 0) {
          this.installDeadline(agent, descriptor, timeoutMs)
        }
  ...
  provision(lead: Agent, request: ProvisionExpertRequest): Promise<ProvisionedExpert> {
    return this.locks.run(lead.id, async () => {
      const resolved = await this.ctx.expertCatalog.resolve(request.blueprint, {
        ...
        ...request.localSkills === undefined ? {} : { skills: request.localSkills },
      ...
      const agentOptions = this.agentOptions(resolved, this.inheritedAgentOptions(lead), request.modelSelection)
      const foundation = this.foundation(lead, resolved, agentOptions, request.modelSelection)
      const publicRole = request.role?.trim() || resolved.blueprint.role
      const descriptor = this.bindingDescriptor(resolved, agentOptions, publicRole, marketplaceSkills, foundation)
  ...
  async followup(
    ...
    try {
      ...
      const priorFailure = latestExecutionFailure(inspected)
      if (priorFailure !== undefined) {
        await this.settleFailure(lead, binding.attemptId, priorFailure)
  ...
  private bindingDescriptor(
    ...
    foundation: NonNullable<ExpertBindingDescriptor['foundation']>,
  ): ExpertBindingDescriptor {
  ...
  /** Resolve the Lead's live daily-session model selection over stale creation options. */
  private inheritedAgentOptions(lead: Agent): AgentOptions {
    const selected = currentModelSelection(lead.ctx)?.current
    return {
      ...lead.options,
  ...
  /** Snapshot the daily-agent infrastructure that the expert must reproduce. */
  private foundation(
    lead: Agent,
    resolved: ResolvedExpertBinding,
    agentOptions: AgentOptions,
    selected?: ModelSelection,
  ): NonNullable<ExpertBindingDescriptor['foundation']> {
    const leadSelection = currentModelSelection(lead.ctx)?.current
    const provider = agentOptions.provider
    const model = agentOptions.model
    if (provider === undefined || model === undefined) {
      throw new TeamRunError('expert model route is unavailable', 'CAPABILITY_UNAVAILABLE')
    }
    const sameLeadRoute = leadSelection?.provider === provider && leadSelection.model === model
    const modelSelection: ModelSelection = selected === undefined
      ? {
        provider,
        model,
        ...sameLeadRoute && leadSelection.reasoningEffort !== undefined
          ? { reasoningEffort: leadSelection.reasoningEffort }
          : {},
      }
      : structuredClone(selected)
    const approval = this.ctx.get('approval')
    const approvalPolicy = approval?.overrideOf(lead.session) ?? approval?.config.policy ?? 'ask'
    const sandboxMode = this.ctx.get('sandboxPolicy')?.resolve({ session: lead.session }).mode ?? 'read-only'
    const tools = resolved.blueprint.tools
    const toolAccess = tools.allow === undefined && (tools.deny?.length ?? 0) === 0
      ? 'full_preset'
      : 'restricted'
    return {
      modelSelection,
      toolAccess,
      permissions: { sandboxMode, approvalPolicy },
    }
  }
  ...
  private agentOptions(
    ...
    const provider = selected?.provider ?? resolved.blueprint.model.provider ?? inherited.provider
    const model = selected?.model ?? resolved.blueprint.model.model ?? inherited.model
    return {
  ...
  private setupChild(child: Agent): () => void {
    ...
    if (existing !== undefined) {
      ...
      return this.mountExpertFoundation(child, expected, false)
    }
    ...
    return this.mountExpertFoundation(child, expected, true)
  }

  /** Install the expert's daily model/permission foundation and task-time marketplace additions. */
  private mountExpertFoundation(
    child: Agent,
    binding: ExpertBindingEventData,
    fresh: boolean,
  ): () => void {
    ...
    try {
      const foundation = binding.descriptor.foundation
      if (foundation !== undefined) {
        if (fresh) {
          const sandbox = this.ctx.get('sandboxPolicy')
          if (sandbox?.overrideOf(child.session) !== foundation.permissions.sandboxMode) {
            child.session.append('sandbox/mode', {
              mode: foundation.permissions.sandboxMode,
              source: 'delegation',
            })
          }
          const approval = this.ctx.get('approval')
          if (approval?.overrideOf(child.session) !== foundation.permissions.approvalPolicy) {
            child.session.append('approval/policy', {
              policy: foundation.permissions.approvalPolicy,
              source: 'delegation',
            })
          }
        }
        const logged = child.session.requestHeader()?.config
        const selected: ModelSelection = logged === undefined
          ? structuredClone(foundation.modelSelection)
          : {
            provider: logged.provider,
            model: logged.model,
            ...logged.reasoningEffort === undefined ? {} : { reasoningEffort: logged.reasoningEffort },
          }
        const installed = currentModelSelection(child.ctx)
        if (installed === undefined) {
          disposers.push(installModelSelection(child.ctx, { current: selected, assembled: undefined }))
        } else {
          const previous = installed.current
          installed.current = selected
          disposers.push(() => {
            if (installed.current === selected) installed.current = previous
          })
        }
      }
      disposers.push(this.mountMarketplaceSkills(child, binding.descriptor.marketplaceSkills ?? []))
      ...
      for (const dispose of disposers.reverse()) {
        try {
          ...
          this.ctx.logger.warn(`expert foundation cleanup failed: ${errorMessage(cleanupError)}`)
        }
      }
      throw error
    }
  ...
  private async assertResolvedBinding(
    ...
    try {
      const retainedSkills = binding.descriptor.skills.map(skill => skill.name)
      resolved = await this.ctx.expertCatalog.resolve(binding.descriptor.blueprint, {
        ...cwd === undefined ? {} : { cwd },
        ...retainedSkills.length === 0 ? {} : { skills: retainedSkills },
    ...
    const expectedOptions = this.agentOptions(
      resolved,
      binding.descriptor.model,
      binding.descriptor.foundation?.modelSelection,
    )
    const displayRole = binding.descriptor.displayRole
    const foundation = binding.descriptor.foundation
    if (foundation !== undefined) {
      const tools = resolved.blueprint.tools
      const expectedToolAccess = tools.allow === undefined && (tools.deny?.length ?? 0) === 0
        ? 'full_preset'
        : 'restricted'
      if (foundation.toolAccess !== expectedToolAccess
        || foundation.modelSelection.provider !== expectedOptions.provider
        || foundation.modelSelection.model !== expectedOptions.model) {
        throw revisionMismatch(binding.descriptor.digest, `runtime-foundation:${resolved.digest}`)
      }
    }
    const expectedDescriptor: ExpertBindingDescriptor = {
      ...
      ...foundation === undefined ? {} : { foundation: structuredClone(foundation) },
      compositionDigest: compositionDigest(
        resolved,
        expectedOptions,
  ...
  /** Settle model failures already durable before an active Lead was restored into this process. */
  private async reconcilePersistedFailures(candidate: Agent): Promise<void> {
    const membership = this.ctx.teamRuns.tryMembership(candidate)
    if (membership?.actor.role !== 'lead') return
    const run = this.ctx.teamRuns.getRun(candidate)
    if (run.phase !== 'provisioning' && run.phase !== 'active' && run.phase !== 'completing') return
    for (const member of run.members) {
      if (member.phase !== 'active') continue
      const inspected = await this.inspectChild(member.sessionId)
      if (inspected === undefined) continue
      const failure = latestExecutionFailure(inspected)
      if (failure === undefined) continue
      await this.settleFailure(candidate, member.attemptId, failure)
      this.clearExecutionBudget(member.sessionId)
    }
  }
  ...
  /** Suspend only active model execution time while a human approval answer is outstanding. */
  private pauseForApproval(childId: SessionId, approvalId: string): void {
    const existing = this.approvalWaits.get(childId)
    if (existing !== undefined) {
      existing.ids.add(approvalId)
      return
    }
    const deadline = this.deadlines.get(childId)
    const remainingMs = deadline === undefined ? undefined : Math.max(0, deadline.deadlineAt - Date.now())
    this.clearDeadline(childId)
    this.approvalWaits.set(childId, { ids: new Set([approvalId]), remainingMs })
  }

  /** Resume the exact remaining execution budget after the final outstanding approval is decided. */
  private resumeAfterApproval(
    agent: Agent,
    descriptor: ExpertChildDescriptorEventData,
    approvalId: string,
  ): void {
    const wait = this.approvalWaits.get(agent.id)
    if (wait === undefined || !wait.ids.delete(approvalId) || wait.ids.size > 0) return
    this.approvalWaits.delete(agent.id)
    if (agent.status !== 'running' || this.parentRunIsTerminal(agent)) return
    const timeoutMs = this.executionTimeouts.get(agent.id)
    if (timeoutMs === undefined) return
    const remainingMs = wait.remainingMs ?? timeoutMs
    ...
    if (!Number.isSafeInteger(deadlineAt)) {
      void this.failFromChild(
        ...
        this.ctx.logger.warn(`expert "${agent.id}" approval deadline resume failed: ${errorMessage(error)}`)
      })
      return
    }
  ...
  private clearExecutionBudget(childId: SessionId): void {
    ...
    this.approvalWaits.delete(childId)
  }
