...
export function renderExpertInitialPrompt(
  ...
  return [
    ...
    'When you hold the baton, execute only the assigned next action. Before marking an owned task complete, write a review-state artifact authored by you that explicitly covers that task. Then route exactly one artifact-linked public handoff or review to exactly one best-suited next recipient through collaboration_followup before ending the turn. A comment without your own artifact is not a completed contribution. ',
