...
import { parseExpertBinding, parseExpertChildDescriptor } from "./schema.js";
/**
 ...
 */
export function sameExpertDescriptor(left, right) {
 ...
 */
export function foldExpertBindings(runId, events) {
    const found = [];
    const attempts = new Set();
    const sessions = new Set();
    for (const event of events) {
        if (event.type !== 'collaboration/expert/binding')
            continue;
 ...
 */
export function findExpertBinding(runId, events, attemptId) {
 ...
 */
export function foldExpertChildDescriptor(session) {
    const own = session.events.slice(session.header.seedLength ?? 0);
    let found;
    for (const event of own) {
        if (event.type !== 'collaboration/expert/descriptor')
            continue;
        const value = parseExpertChildDescriptor(event.data);
        if (found !== undefined)
            throw new Error('persisted expert child has more than one descriptor');
        found = value;
    }
    return found;
}
 ...
 */
export function countExpertTurns(session) {
    const own = session.events.slice(session.header.seedLength ?? 0);
 ...
 */
export function hasExpertInitialPrompt(session, prompt) {
