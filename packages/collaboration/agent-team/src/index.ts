...
import { applyTeamRunEvent, snapshotTeamRun } from './fold.ts'
...
export class TeamRunService extends Service {
  ...
  /**
   * Atomically publish a bounded batch of independently addressed public messages.
   * @param caller - exact live Lead or active expert author.
   * @param requests - complete messages validated in order against one staged TeamRun state.
   * @returns committed public messages in request order.
   */
  async publishMessages(
    caller: Agent,
    requests: readonly PublishCollaborationMessageRequest[],
  ): Promise<PublicCollaborationMessage[]> {
    if (requests.length === 0) {
      throw new TeamRunError('public message batch must not be empty', 'TEAM_INVALID_ARGUMENT')
    }
    const initial = this.authority(caller)
    return this.journal.transact(initial.lead.id, async () => {
      const staged = this.journal.state(initial.lead)
      const startedAt = Date.now()
      const ids: Array<ReturnType<typeof CollaborationMessageId>> = []
      const inputs: Array<{
        readonly type: 'collaboration/message'
        readonly data: import('@deepseek-ai/dsh-session').SessionEventMap['collaboration/message']
      }> = []
      for (const request of requests) {
        const targets = (request.targets ?? []).map(name => resolveActorByName(staged, name))
        this.assertMessageAdmission(staged, initial.membership.actor, request, targets)
        const id = CollaborationMessageId(`collaboration-message-${randomUUID()}`)
        const data = {
          version: 1 as const,
          runId: staged.id,
        ...
        const input = { type: 'collaboration/message' as const, data }
        applyTeamRunEvent(staged, {
          ...input,
          seq: initial.lead.session.events.length + inputs.length,
          time: startedAt + inputs.length,
        })
        ids.push(id)
        inputs.push(input)
      }
      await this.journal.appendBatchAndFlush(initial.lead, inputs)
      const committed = this.journal.state(initial.lead)
      return ids.map(id => structuredClone(committedValue(
        committed.messages.get(id),
        `public message "${id}"`,
      )))
    })
  }
