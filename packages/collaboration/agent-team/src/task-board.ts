...
import { acceptedOwnerArtifact } from './completion-evidence.ts'
    ...
    switch (request.action) {
      ...
      case 'complete':
        ...
        if (state.protocol !== undefined) {
          if (!leadAuthority) {
            throw new TeamRunError(
              `only the TeamRun Lead can confirm completion of enforced task "${current.id}"`,
              'TEAM_LEAD_REQUIRED',
            )
          }
          if (acceptedOwnerArtifact(state, current) === undefined) {
            throw new TeamRunError(
              `team task "${current.id}" requires its owner's routed artifact to be accepted before completion`,
              'TEAM_TASK_INVALID_TRANSITION',
            )
          }
        }
