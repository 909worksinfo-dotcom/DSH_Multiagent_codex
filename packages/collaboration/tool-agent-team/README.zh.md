...
稳定 [`ctx.teamRuns`](../agent-team/README.md) 领域的 scoped 模型适配器。它只在 live Lead 或 active 专家 scope 中安装一份公开协作策略和十五个有界工具。它永远不拥有 formation、provisioning、lifecycle、task、ledger 或 message 状态，因此领域服务仍是唯一真相源
...
- `collaboration_followup` 在发送者进入空闲后，把一个串行回合推进给唯一最合适的接收者
- `collaboration_parallel_followup` 原子派发一个明确并行 Charter 阶段中的至少两个就绪任务，并发启动对应专家 inbox，并在末尾只汇合一次
...
串行阶段只在当前发送者进入空闲状态后推进。路由器会重新读取权威 run、总结相关上下文、规划一个下一步动作、选择唯一一名符合条件的接收者，并将公开内容生成为上下文摘要、下一步、接收者选择和消息四类结构化字段。`task` 路由必须引用一个已就绪的 Charter 任务，并以其预分配的 active 专家为接收者，因此下游阶段无法在 blocker 完成前启动

只有 committed mode 为 `parallel` 的 Charter 阶段可以使用 `collaboration_parallel_followup`。Lead 会在一个原子公开批次中提交至少两个不同的就绪任务与 owner 组合。inbox admission 通过 `Promise.allSettled` 并发运行，只有成功进入 inbox 的专家获得执行许可；每位专家必须把完全相同的任务回交 Lead，Lead 在 barrier 汇合前保持阻塞，最后只收到一次包含全部成功回交和初始投递失败的汇合通知。当该明确并行阶段仍有两个或以上就绪任务时，单条串行任务路由会被拒绝。共享 thread 与公开路由支持 live recovery 重建未完成批次，同时不会改写不可变历史消息

这些结构化字段无需依赖展示性移交前缀即可支持恢复和清晰投影。恢复逻辑同时识别当前结构化字段与旧存储格式，因此不可变历史任务仍可继续执行，新生成消息不会包含旧标记
...
一段固定策略，加一行简短的当前成员 TeamRun role、公开名称和 run id。包括 `collaboration_get` 在内的十五个 collaboration schema 只出现在当前成员 scope 中。run value 包含权威 protocol topology、limit、member 权限、可用目标、已用与剩余预算和 challenge round。工具调用返回紧凑 canonical JSON，send receipt 和 run 投影不会重复 artifact 正文

#### Token 影响

固定策略和十五个 schema 会为每次 TeamRun 成员请求增加有界成本。read 和 mutation 调用会在调用后追加紧凑的 run、task、ledger、controller、page 或 message-receipt JSON。较大的公开 content 和 artifact 正文不会出现，除非模型显式调用对应的受限读取
