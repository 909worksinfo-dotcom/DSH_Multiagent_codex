 ...
 */
export type EscalationOutcome = 'allowed-once' | 'allowed-for-turn' | 'rejected' | 'cancelled' | 'unavailable'
...
export interface EscalationApprover<A = object, C = string> {
   ...
   */
  request(req: { agent: A; toolName: string; callId: C; reason: string; taskKey: string; signal?: AbortSignal }): Promise<EscalationOutcome>
}
...
export async function approveEscalation<A, C>(request: EscalationRequest, approval: EscalationApproval<A, C>): Promise<SandboxMode> {
  ...
  const outcome = await approval.approver.request({
    ...
    taskKey: `sandbox-escalation:${subject}:${mode}`,
  ...
  switch (outcome) {
    ...
    case 'allowed-once':
    case 'allowed-for-turn': return mode as SandboxMode
