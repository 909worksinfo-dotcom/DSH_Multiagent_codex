...
/** Exact Agent scope to its installed mutable model-selection owner. */
const installedSelections = new WeakMap<Context, ModelSelectionRef>()

/**
 * Read the model-selection owner already installed for one Agent scope.
 * Runtime extensions use this to inherit the same live route instead of
 * copying the Agent's stale creation-time provider/model fields.
 * @param agentCtx - exact Agent-scoped context.
 * @returns the installed mutable selection, or `undefined` before composition.
 */
export function currentModelSelection(agentCtx: Context): ModelSelectionRef | undefined {
...
export function installModelSelection(agentCtx: Context, selection: ModelSelectionRef): () => void {
  const installed = installedSelections.get(agentCtx)
  if (installed !== undefined) {
    if (installed === selection) return () => undefined
    throw new Error('agent scope already owns a different model selection')
  }
