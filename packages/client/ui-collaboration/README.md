...
The task board renders dependency readiness, ownership, progress, resource scopes, and conflicts from the runtime projection. The public timeline renders the typed collaboration vocabulary, including viewpoints, challenges, responses, reviews, decisions, artifacts, and the lead agent's final delivery. It explicitly excludes private model reasoning. Final delivery is highlighted only when an authoritative `final_delivery` event exists
