...
function binding(index: number) {
  return {
    ...
    foundation: {
      model: { mode: 'inherit_lead', maxTokens: 8_192 },
      tools: { access: 'full_preset', allow: [], deny: [] },
      permissions: { sandboxMode: 'inherit_lead', approvalPolicy: 'inherit_lead' },
    },
    skills: [
      { id: 'collaboration-research-analysis', label: '深度研究与证据分析' },
      { id: 'collaboration-peer-review', label: '协作质疑与同行评审' },
      { id: 'collaboration-product-solution', label: '产品方案设计' },
    ],
...
function charter(maxExperts: number, topology: 'centralized' | 'grouped') {
  return {
    ...
    tasks: [
      { id: 'scope', subject: '收敛任务边界', description: '核对用户目标、业务背景、关键假设与验收标准', blockedBy: [], assigneeSlotId: 'slot-1' },
      { id: 'evidence', subject: '完成专业分析', description: '基于任务类型形成有证据、可追溯的专业分析资产', blockedBy: [], assigneeSlotId: 'slot-2' },
      { id: 'challenge', subject: '交叉质疑与补强', description: '检查逻辑漏洞、反例、风险和未被证据支持的结论', blockedBy: ['scope', 'evidence'], assigneeSlotId: 'slot-3' },
    ],
  } as const
}
...
export function createCollaborationDemoPort(seed = createDemoRuns()): CollaborationPort {
  ...
  const create = (request: CreateCollaborationRunRequest, prior?: CollaborationRunSnapshot): CollaborationRunSnapshot => {
    ...
    const experts = Array.from({ length: planned }, (_, index) => ({
      ...expert(index + 1, 'planned'),
      id: `slot-${index + 1}`,
      sessionId: null,
    }))
    return {
      id, title: request.title, objective: request.objective, language,
      status: 'forming', phase: 'planning', createdAt: Date.now(), cursor: 0,
      ...
      experts,
      expertCounts: { planned, provisioning: 0, active: 0, failed: 0, attempts: 0 },
      tasks: [], timeline: [], artifacts: [], decisions: [], qualityGates: [],
      controller: emptyController(), progress: emptyProgress(), protocol: legacyProtocol(),
    }
  }
  return {
    ...
    confirmRun: (runId) => {
      const plannedRun = snapshot.runs.find(run => run.id === runId)
      const plannedCharter = plannedRun?.charter
      if (plannedRun === undefined || plannedRun.phase !== 'planning' || plannedCharter === null || plannedCharter === undefined) {
        return Promise.reject(new Error('Run is not awaiting confirmation'))
      }
      const experts = plannedRun.experts.map((value, index) => ({
        ...value,
        id: `${runId}-attempt-${index + 1}`,
        sessionId: `${runId}-expert-${index + 1}`,
        phase: 'active' as const,
      }))
      const expertBySlot = new Map(plannedRun.experts.map((value, index) => [value.id, experts[index]]))
      const tasks: CollaborationTask[] = plannedCharter.tasks.map((task, index) => {
        const assigned = task.assigneeSlotId === null ? undefined : expertBySlot.get(task.assigneeSlotId)
        return {
          id: `task-${index + 1}`,
          revision: 2,
          ...
          status: 'pending',
          owner: assigned === undefined ? null : {
            role: 'expert', memberId: assigned.id, sessionId: assigned.sessionId, name: assigned.name,
          },
          blockedBy: task.blockedBy.map(id => `task-${plannedCharter.tasks.findIndex(value => value.id === id) + 1}`),
          resourceScopes: [], ready: task.blockedBy.length === 0, resourceConflicts: [],
        }
      })
      publish(snapshot.runs.map(run => run.id === runId ? {
        ...run, phase: 'active', status: 'running', experts,
        expertCounts: { planned: experts.length, provisioning: 0, active: experts.length, failed: 0, attempts: experts.length },
        tasks, progress: { ...run.progress, total: tasks.length, ready: tasks.filter(task => task.ready).length },
      } : run))
