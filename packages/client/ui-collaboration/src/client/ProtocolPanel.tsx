  ...
  return (
    <section className={css.protocol} aria-labelledby="collaboration-protocol-title">
      <header className={css.heading}>
        ...
        <span className={css.topology}>{protocol.topology === null ? copy(language, 'protocol.unavailable') : copy(language, `topology.${protocol.topology}`)}</span>
      </header>
      <p className={css.authority}><span aria-hidden="true">◎</span>{copy(language, 'protocol.authoritative')}</p>

      <div className={css.summaryGrid}>
        <section className={css.rulePanel} aria-labelledby="protocol-runtime-rules">
          <div className={css.panelHeading}><div><span className={css.enforcedMark}>{copy(language, 'protocol.enforced')}</span><h4 id="protocol-runtime-rules">{copy(language, 'protocol.rules')}</h4></div><strong>{protocol.topology === null ? copy(language, 'protocol.unavailable') : copy(language, `topology.${protocol.topology}`)}</strong></div>
          {protocol.topology === null || limits === null
            ? <p className={css.muted}>{copy(language, 'protocol.rules.unavailable')}</p>
            : <ol>
              <li><span>01</span><p>{copy(language, `protocol.topologyRule.${protocol.topology}`)}</p></li>
              <li><span>02</span><p>{copy(language, 'protocol.limitRule.challenge', { count: limits.maxChallengeRounds })}</p></li>
              <li><span>03</span><p>{copy(language, 'protocol.limitRule.messages', { count: limits.maxMessagesPerExpert })}</p></li>
            </ol>}
        </section>
        <section className={css.budgetPanel} aria-labelledby="protocol-budget-title">
      ...
      <section className={css.section} aria-labelledby="protocol-members-title">
      ...
      <section className={css.section} aria-labelledby="protocol-challenges-title">
