...
import type {
  CollaborationApprovalOutcome, CollaborationExpertApproval, CollaborationRootProps,
  CollaborationTriggerProps,
} from './contract.ts'
...
import { preferredCollaborationRun } from './run-selection.ts'
  ...
  return (
    <section className={css.binding} aria-label={copy(language, 'member.binding')}>
      ...
      <dl>
        ...
        <div className={css.bindingGroup}><dt>{copy(language, 'member.model')}</dt><dd><span>{foundation === undefined
          ? copy(language, 'member.foundation.legacy')
          : foundation.model.mode === 'inherit_lead'
            ? copy(language, 'member.foundation.model.inherit')
            : `${foundation.model.provider ?? '—'} / ${foundation.model.model ?? '—'}`}</span></dd></div>
        <div className={css.bindingGroup}><dt>{copy(language, 'member.tools')}</dt><dd><span>{foundation === undefined
          ? copy(language, 'member.foundation.legacy')
          : foundation.tools.access === 'full_preset'
            ? copy(language, 'member.foundation.tools.full')
            : copy(language, 'member.foundation.tools.restricted')}</span></dd></div>
        <div className={css.bindingGroup}><dt>{copy(language, 'member.permissions')}</dt><dd><span>{foundation === undefined
          ? copy(language, 'member.foundation.legacy')
          : foundation.permissions.sandboxMode === 'inherit_lead'
            ? copy(language, 'member.foundation.permissions.inherit')
            : `${foundation.permissions.sandboxMode} · ${copy(language, `member.foundation.approval.${foundation.permissions.approvalPolicy}`)}`}</span></dd></div>
...
function RunDetail({ run, language, tab, selectedMemberId, actions }: {
  ...
  const showRunNotice = run.status === 'team_formation_failed' || invalidReadyTeam || run.status === 'cancelled'
  return (
    <section className={css.workspace} aria-label={copy(language, 'title')}>
      {showRunNotice ? <header className={css.runHeader}>
...
function ExpertApprovalPanel({ approval, expert, run, language, onAnswer }: {
  approval: CollaborationExpertApproval
  expert: CollaborationExpertMember
  run: CollaborationRunSnapshot
  language: CollaborationLanguage
  onAnswer: (outcome: CollaborationApprovalOutcome) => Promise<void>
}) {
  const [answering, setAnswering] = useState<CollaborationApprovalOutcome | null>(null)
  ...
  const answer = (outcome: CollaborationApprovalOutcome): void => {
    if (answering !== null) return
    setAnswering(outcome)
    setError(false)
    void onAnswer(outcome).catch(() => {
      setAnswering(null)
      setError(true)
    })
  }
  const expertName = collaborationRunParticipantName(run, expert.name, 'expert', language)
  const taskGrantAvailable = approval.collaborationRunId === run.id
  return (
    <section className={css.expertApproval} data-collaboration-expert-approval={approval.key} role="alert">
      <div className={css.expertApprovalMark} aria-hidden="true">!</div>
      <div className={css.expertApprovalBody}>
        <div className={css.expertApprovalHeading}>
          <strong>{copy(language, 'approval.title')}</strong>
          <span>{copy(language, 'approval.expert', { expert: expertName })}</span>
        </div>
        <p>{approval.reason ?? copy(language, 'approval.reason', { tool: approval.toolName })}</p>
        <small>{copy(language, 'approval.tool', { tool: approval.toolName })}</small>
        {error ? <em>{copy(language, 'approval.error')}</em> : null}
      </div>
      <div className={css.expertApprovalActions}>
        <button type="button" disabled={answering !== null} onClick={() => { answer('rejected') }}>{copy(language, 'approval.reject')}</button>
        <button type="button" disabled={answering !== null} onClick={() => { answer('allowed-once') }}>{copy(language, 'approval.allowOnce')}</button>
        <button type="button" disabled={answering !== null} onClick={() => { answer('allowed-for-turn') }}>{copy(language, 'approval.allowTurn')}</button>
        {taskGrantAvailable ? (
          <button type="button" data-primary="true" disabled={answering !== null} onClick={() => { answer('allowed-for-task') }}>
            {copy(language, 'approval.allowTask')}
          </button>
        ) : null}
      </div>
    </section>
...
export function CollaborationTrigger({ wide, enterCollaboration }: CollaborationTriggerProps) {
  return (
    <Tooltip label={zh['trigger.aria']} disabled={wide}>
      <button type="button" className={wide ? css.trigger : css.railTrigger} aria-label={zh['trigger.aria']} onClick={enterCollaboration}>
        <IconBranchOutline16 size={wide ? 16 : 18} aria-hidden="true" />
        {wide ? <span>{zh.trigger}</span> : null}
      </button>
    </Tooltip>
  )
}
...
export function CollaborationRoot({
  open, useStore, actions, useSessions, useCollaboration, readExpertApproval,
  answerExpertApproval, refreshCollaboration, openCollaboration, closeCollaboration, t,
  ...
  const run = preferredCollaborationRun(catalog.runs)
  const language = run?.language ?? detectCollaborationLanguage(t('title'))
  const approvalSignal = useSessions(value => Object.values(value.byId)
    .filter(session => session.pendingInteraction === 'approval')
    .map(session => String(session.id))
    .sort()
    .join('|'))
  const pendingApproval = useMemo(() => {
    for (const expert of run?.experts ?? []) {
      if (expert.sessionId === null) continue
      try {
        const approval = readExpertApproval(expert.sessionId)
        if (approval !== null) return { expert, approval }
      } catch (error: unknown) {
        console.error(`[ui-collaboration] failed to read approval for expert session ${expert.sessionId}:`, error)
      }
    }
    return null
  }, [approvalSignal, readExpertApproval, run])
  const pendingExpert = pendingApproval?.expert
  const expertApproval = pendingApproval?.approval ?? null
  ...
  useEffect(() => {
    if (open || expertApproval === null) return
    openCollaboration()
  }, [expertApproval, open, openCollaboration])
  ...
  useEffect(() => {
    ...
    const sync = (): void => {
      ...
      busy = true
      void refreshRef.current(runId).finally(() => { busy = false })
    }
    sync()
      ...
      <header className={css.dockHeader}>
        <div className={css.dockTitle}>
          ...
          <div>
            ...
            {run === undefined ? null : <span className={css.headerStatus} data-tone={expertApproval === null ? statusTone(run.status) : 'live'} aria-live="polite">{expertApproval === null ? copy(language, `status.${run.status}`) : copy(language, 'approval.status')}</span>}
          </div>
        </div>
      ...
      <main className={css.dockBody} aria-busy={catalog.state === 'loading'}>
        {catalog.state !== 'ready'
          ...
          : run === undefined
            ...
            : <div className={css.runDetailStack}>
              {expertApproval === null || pendingExpert === undefined ? null : (
                <ExpertApprovalPanel
                  approval={expertApproval}
                  expert={pendingExpert}
                  run={run}
                  language={language}
                  onAnswer={outcome => answerExpertApproval(expertApproval.sessionId, expertApproval.key, outcome)}
                />
              )}
              <RunDetail
                run={run}
