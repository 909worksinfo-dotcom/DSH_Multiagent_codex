...
describe('P5 collaboration preview fixture', () => {
  it('covers minimum-team formation and an eight-expert running team', () => {
  ...
  it('retries formation by creating a new reviewable TeamRun without mutating the failed audit terminal', async () => {
    ...
    expect(after.runs.find(run => run.id === newId)).toMatchObject({ status: 'forming', phase: 'planning' })
  })
})
