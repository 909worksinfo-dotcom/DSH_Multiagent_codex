...
import type {
  ModelCatalogFailure, ModelProviderGroup, ModelSelection, SessionId,
...
export interface CollaborationTaskProfile {
  /** Host-classified task domain; absent only on snapshots from older clients. */
  readonly domain?: 'research_analysis' | 'product_solution' | 'software_development'
...
export interface CollaborationTeamCharter {
  ...
  readonly tasks: readonly CollaborationPlannedTask[]
}

/** One immutable pre-execution task and its planned expert seat. */
export interface CollaborationPlannedTask {
  readonly id: string
  readonly subject: string
  readonly description: string
  readonly blockedBy: readonly string[]
  readonly assigneeSlotId: string | null
}
...
export interface CollaborationSafeExpertBindingMetadata {
  ...
  readonly foundation?: {
    readonly model: {
      readonly mode: 'inherit_lead' | 'selected'
    ...
    readonly tools: {
      readonly access: 'full_preset' | 'restricted'
      readonly allow: readonly string[]
      readonly deny: readonly string[]
    }
    readonly permissions: {
      readonly sandboxMode: 'inherit_lead' | 'read-only' | 'workspace-write' | 'danger-full-access'
      readonly approvalPolicy: 'inherit_lead' | 'ask' | 'never'
    }
  }
}
...
export interface CreateCollaborationRunRequest {
  ...
  readonly objective: string
  readonly language: CollaborationLanguage
  /** Exact daily-session route selected for the Lead coordinator. */
  readonly leadModel?: ModelSelection
  /** Exact daily-session routes selected for reviewed deterministic roster slots. */
  readonly expertModels?: readonly CollaborationExpertModelSelection[]
}

/** One reviewed expert slot and its exact provider-owned model route. */
export interface CollaborationExpertModelSelection {
  readonly slotId: string
  readonly selection: ModelSelection
}

/** The same provider/model directory exposed by an everyday session. */
export interface CollaborationModelCatalog {
  readonly current: ModelSelection
  readonly groups: readonly ModelProviderGroup[]
  readonly failures: readonly ModelCatalogFailure[]
}

/** Model-invocable local skill offered for one reviewed expert binding. */
export interface CollaborationSkillOption {
  readonly id: string
  readonly label: string
  readonly description: string
}

/** Fixed secret references used by collaboration external capabilities. */
export type CollaborationExternalCredentialRef =
  | 'SMITHERY_API_KEY'
  | 'COMPOSIO_API_KEY'

/** Value-free credential readiness safe for the planning UI. */
export interface CollaborationExternalCredential {
...
export interface ICollaboration {
  ...
  /** List the Lead preset's model-invocable skills for review-time expert configuration. */
  listSkills(runId: CollaborationRunId): Promise<readonly CollaborationSkillOption[]>
  /** Load the Lead's daily-session provider/model directory for expert review. */
  listModels(runId: CollaborationRunId): Promise<CollaborationModelCatalog>
  /** Describe collaboration credentials without returning any secret value. */
  describeExternalCredentials(): Promise<readonly CollaborationExternalCredential[]>
  /** Store one allowlisted collaboration credential through the shared daily-session credential service. */
  setExternalCredential(ref: CollaborationExternalCredentialRef, value: string): Promise<void>
  createRun(request: CreateCollaborationRunRequest): Promise<CollaborationRunId>
  /** Provision the exact reviewed roster and begin Lead execution. */
  confirmRun(runId: CollaborationRunId): Promise<void>
