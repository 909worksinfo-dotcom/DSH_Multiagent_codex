...
function wireRun(
  ...
  return {
    ...
    charter: {
      ...
      taskDag: [{
        id: 'analysis', subject: '分析', description: '分析需求', blockedBy: [],
        requiredCapabilities: ['analysis'], resourceScopes: [], assigneeSlotId: 'slot-1',
      }],
    ...
    experts: [{
      ...
      binding: {
        ...
        foundation: {
          model: { mode: 'selected', provider: 'deepseek', model: 'deepseek-chat', maxTokens: 4_096 },
          tools: { access: 'full_preset', allow: [], deny: [] },
          permissions: { sandboxMode: 'workspace-write', approvalPolicy: 'ask' },
        },
...
function wireExpertHandoff(cursor: number): CollaborationPublicEventView {
  return {
    id: `message-${String(cursor)}`,
    eventId: `event-${String(cursor)}`,
    cursor,
    ...
    kind: 'handoff',
    author: {
      role: 'expert', memberId: 'member-1', sessionId: 'expert-1' as SessionId, name: 'expert-1',
    },
    targets: [{ role: 'lead', sessionId: 'lead-verified' as SessionId, name: 'lead' }],
    references: { taskId: 'task-1', artifactId: 'artifact-1' },
    content: 'The owner returned the completed artifact to the Lead.',
    createdAt: 1_000 + cursor,
    visibility: 'public',
  }
}
...
describe('CollaborationRuntime', () => {
  it('recovers the authoritative catalog and projects only UI business fields', async () => {
    ...
    expect(snapshot.runs[0]?.experts[0]?.binding.foundation).not.toBe(run.experts[0]?.binding.foundation)
  })

  it('does not present a persisted task as completed before accepted routed owner evidence exists', async () => {
    const { api, runtime } = harness()
    const run = wireRun('lead-premature' as SessionId, 'request-premature', {
      tasks: [{
        id: 'task-1', revision: 3, subject: '分析', description: '分析需求', status: 'completed',
        owner: { role: 'expert', memberId: 'member-1', sessionId: 'expert-1' as SessionId, name: 'expert-1' },
        blockedBy: [], resourceScopes: [], ready: false, resourceConflicts: [],
      }],
      progress: {
        total: 1, ready: 0, inProgress: 0, completed: 1, blocked: 0, messageCount: 0,
        artifactCount: 1, decisionCount: 1, qualityGatePending: 0, qualityGatePassed: 1, qualityGateFailed: 0,
      },
    })
    api.onCollaborationList = () => Promise.resolve(ok({ runs: [run] }))
    ...
    expect(runtime.source.getSnapshot().runs[0]).toMatchObject({
      tasks: [{ id: 'task-1', status: 'in_progress' }],
      progress: { total: 1, inProgress: 1, completed: 0 },
    })
  })

  it('presents a task as completed after the accepted owner artifact was routed to the Lead', async () => {
    const { api, runtime } = harness()
    const run = wireRun('lead-verified' as SessionId, 'request-verified', {
      cursor: 2,
      tasks: [{
        id: 'task-1', revision: 4, subject: '分析', description: '分析需求', status: 'completed',
        owner: { role: 'expert', memberId: 'member-1', sessionId: 'expert-1' as SessionId, name: 'expert-1' },
        blockedBy: [], resourceScopes: [], ready: false, resourceConflicts: [],
      }],
      artifacts: [{
        id: 'artifact-1', version: 2, kind: 'analysis', title: '分析资产', status: 'accepted',
        author: { role: 'expert', memberId: 'member-1', sessionId: 'expert-1' as SessionId, name: 'expert-1' },
        taskIds: ['task-1'], mediaType: 'text/markdown', updatedAt: 1_010,
      }],
      progress: {
        total: 1, ready: 0, inProgress: 0, completed: 1, blocked: 0, messageCount: 1,
        artifactCount: 1, decisionCount: 1, qualityGatePending: 0, qualityGatePassed: 1, qualityGateFailed: 0,
      },
    })
    api.onCollaborationList = () => Promise.resolve(ok({ runs: [run] }))
    api.onCollaborationEvents = () => Promise.resolve(ok({
      events: [wireExpertHandoff(1)], hasMore: false, nextCursor: 2,
    }))
    ...
    expect(runtime.source.getSnapshot().runs[0]).toMatchObject({
      ...
      progress: { total: 1, inProgress: 0, completed: 1 },
    })
  })
  ...
  it('hydrates only the currently relevant run instead of flooding the transport with historical timelines', async () => {
    const { api, runtime } = harness()
    const current = wireRun('lead-current' as SessionId, 'request-current', { cursor: 2, createdAt: 2_000 })
    const historical = wireRun('lead-historical' as SessionId, 'request-historical', { cursor: 2, createdAt: 1_000 })
    const eventRequests: SessionId[] = []
    api.onCollaborationList = () => Promise.resolve(ok({ runs: [current, historical] }))
    api.onCollaborationEvents = (payload) => {
      eventRequests.push(payload.runId)
      return Promise.resolve(ok({ events: [wireEvent(1, 'proposal', '最新任务已恢复')], hasMore: false, nextCursor: 2 }))
    }
    ...
    expect(runtime.source.getSnapshot()).toMatchObject({
      state: 'ready',
      runs: [
        { id: current.id, timeline: [{ content: '最新任务已恢复' }] },
        { id: historical.id, timeline: [] },
      ],
    })
    expect(eventRequests).toEqual([current.id])
  })

  it('bounds boot hydration to the visible and active runs even with a large persisted catalog', async () => {
    const { api, runtime } = harness()
    const completed = wireRun('lead-completed' as SessionId, 'request-completed', {
      cursor: 0, createdAt: 3_000, status: 'completed', phase: 'completed',
    })
    const active = wireRun('lead-active' as SessionId, 'request-active', {
      cursor: 0, createdAt: 2_000, status: 'running', phase: 'active',
    })
    const historical = Array.from({ length: 83 }, (_, index) => wireRun(
      `lead-historical-${String(index)}` as SessionId,
      `request-historical-${String(index)}`,
      { cursor: 0, createdAt: 1_000 - index, status: 'completed', phase: 'completed' },
    ))
    const eventRequests: SessionId[] = []
    const listRequests: unknown[] = []
    api.onCollaborationList = (payload) => {
      listRequests.push(payload)
      return Promise.resolve(ok({ runs: [completed, active, ...historical] }))
    }
    api.onCollaborationEvents = (payload) => {
      eventRequests.push(payload.runId)
      return Promise.resolve(ok({ events: [], hasMore: false, nextCursor: 0 }))
    }

    await runtime.refresh()

    expect(runtime.source.getSnapshot()).toMatchObject({ state: 'ready', runs: { length: 85 } })
    expect(listRequests).toEqual([{ limit: 20 }])
    expect(eventRequests).toEqual([completed.id, active.id])
  })
  ...
  it('creates a reviewable plan without prompting, then forms and starts the Lead after confirmation', async () => {
    ...
    api.onCollaborationCreate = payload => Promise.resolve(ok(wireRun(payload.leadSessionId, payload.requestId, {
      status: 'forming', phase: 'planning', tasks: [],
      experts: wireRun(payload.leadSessionId, payload.requestId).experts.map(expert => ({
        ...expert, id: 'slot-1', sessionId: null, phase: 'planned',
      })),
      expertCounts: { planned: 2, provisioning: 0, active: 0, failed: 0, attempts: 0, availableSlots: 8 },
    })))

    const id = await runtime.createRun({
      title: '验证协作组队', objective: '分析需求并完成开发和测试', language: 'zh',
      leadModel: { provider: 'openai', model: 'gpt-5' },
      expertModels: [{
    ...
    expect(api.calls.find(call => call.method === 'collaboration.create')?.payload).toMatchObject({
      leadModel: { provider: 'openai', model: 'gpt-5' },
      expertModels: [{
        slotId: 'slot-1', selection: { provider: 'deepseek-official', model: 'deepseek-chat', reasoningEffort: 'medium' },
      }],
    ...
    expect(runtime.source.getSnapshot().runs[0]).toMatchObject({
      id, status: 'forming', phase: 'planning',
      charter: { tasks: [{ id: 'analysis', assigneeSlotId: 'slot-1' }] },
    })
    api.onCollaborationConfirm = payload => Promise.resolve(ok(wireRun(payload.runId, payload.requestId)))
    await runtime.confirmRun(id)
    ...
    expect(leadPrompt).toContain('由主协调智能体使用 collaboration_task_update 把该任务确认成 completed')
    ...
    expect(leadPrompt).toContain('连同 task_id 发送给其预分配 owner')
    expect(leadPrompt).toContain('使用 collaboration_parallel_followup 一次发送该阶段至少两个')
    expect(leadPrompt).toContain('在全部已启动专家回交后一次性唤醒你')
    ...
    const binding = runtime.source.getSnapshot().runs[0]?.experts[0]?.binding
    expect(binding?.marketplaceProviders?.some(provider =>
      provider.source === 'smithery' && provider.state === 'authorization_required')).toBe(true)
    expect(binding?.marketplaceSkills).toMatchObject([
      { label: 'frontend-design', source: 'skills_sh', status: 'loaded' },
    ])
  })

  it('lists only model-invocable skills for a known review draft', async () => {
    const { api, runtime } = harness()
    api.onCollaborationCreate = payload => Promise.resolve(ok(wireRun(payload.leadSessionId, payload.requestId, {
      status: 'forming', phase: 'planning', tasks: [],
    })))
    const id = await runtime.createRun({
      title: '配置专家技能', objective: '确认专家技能后再执行', language: 'zh',
    })
    api.onSkillList = () => Promise.resolve(ok({ skills: [
      { name: 'z-review', description: '评审证据', modelInvocable: true },
      { name: 'user-only', description: '仅供用户调用', modelInvocable: false },
      { name: 'a-research', description: '检索并分析证据', modelInvocable: true },
    ] }))

    await expect(runtime.listSkills(id)).resolves.toEqual([
      { id: 'a-research', label: 'a-research', description: '检索并分析证据' },
      { id: 'z-review', label: 'z-review', description: '评审证据' },
    ])
    await expect(runtime.listSkills('missing-run')).rejects.toThrow(/unknown run/u)
    expect(api.calls.at(-1)).toMatchObject({ method: 'skill.list', payload: { sessionId: id } })
  })

  it('loads the everyday-session model directory for a known review draft', async () => {
    const { api, runtime } = harness()
    api.onCollaborationCreate = payload => Promise.resolve(ok(wireRun(payload.leadSessionId, payload.requestId, {
      status: 'forming', phase: 'planning', tasks: [],
    })))
    const id = await runtime.createRun({
      title: '配置专家模型', objective: '为专家选择不同模型后再执行', language: 'zh',
    })
    api.onModels = () => Promise.resolve(ok({
      ...
      groups: [{
        id: 'deepseek-official', name: 'DeepSeek', models: [{
          ...
          reasoning: { defaultEffort: 'medium', efforts: [{ id: 'medium', name: '中' }] },
        }],
      }],
      failures: [],
    }))

    await expect(runtime.listModels(id)).resolves.toMatchObject({
      current: { provider: 'deepseek-official', model: 'deepseek-chat', reasoningEffort: 'medium' },
      groups: [{ id: 'deepseek-official', models: [{ id: 'deepseek-chat' }] }],
    })
    await expect(runtime.listModels('missing-run')).rejects.toThrow(/unknown run/u)
    expect(api.calls.at(-1)).toMatchObject({ method: 'session.models', payload: { sessionId: id } })
  })

  it('describes and stores only the fixed collaboration credential references without retaining values', async () => {
    const { api, runtime } = harness()
    const describe = vi.spyOn(api.credentials, 'describe').mockImplementation(_payload => Promise.resolve(ok({ credentials: {
      SMITHERY_API_KEY: { configured: true, writable: true, source: 'file' },
      COMPOSIO_API_KEY: { configured: false, writable: false },
    } })))
    const set = vi.spyOn(api.credentials, 'set').mockImplementation(() => Promise.resolve(ok({})))

    await expect(runtime.describeExternalCredentials()).resolves.toEqual([
      { ref: 'SMITHERY_API_KEY', configured: true, writable: true, source: 'file' },
      { ref: 'COMPOSIO_API_KEY', configured: false, writable: false },
    ])
    expect(describe).toHaveBeenCalledWith({
      refs: ['SMITHERY_API_KEY', 'COMPOSIO_API_KEY'],
    })

    await runtime.setExternalCredential('SMITHERY_API_KEY', '  stored-secret  ')
    expect(set).toHaveBeenCalledWith({ ref: 'SMITHERY_API_KEY', value: 'stored-secret' })
    expect(JSON.stringify(runtime.source.getSnapshot())).not.toContain('stored-secret')
    await expect(runtime.setExternalCredential('DEEPSEEK_API_KEY' as never, 'search-secret'))
      .rejects.toThrow(/unsupported/u)
    await expect(runtime.setExternalCredential('SMITHERY_API_KEY', '   ')).rejects.toThrow(/non-blank/u)
  })
  ...
  it('retains the committed TeamRun when the Lead prompt is rejected', async () => {
    ...
    await expect(runtime.confirmRun(runId)).rejects.toBeInstanceOf(Error)
