...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    requires: [
      'ctx.tools', 'ctx.systemPrompt', 'ctx.teamRuns', 'ctx.expertRuntime', 'ctx.teamOrchestrator',
      'an exact live TeamRun member Agent',
    ],
    ...
    async mount(ctx) {
      ...
      ctx.provide('expertRuntime', { followup: () => Promise.resolve() } as never)
      ctx.provide('teamOrchestrator', { get: () => ({ plan: undefined }) } as never)
      const session = ctx.sessions.create(SessionId('tool-catalog-team-run-lead'))
      const agent = await registerCatalogAgent(ctx, session.id, [
        'tools', 'systemPrompt', 'agents', 'teamRuns', 'expertRuntime', 'teamOrchestrator',
      ])
    ...
    scope: ctx => catalogChildScopes.get(ctx) as Agent,
    note:
      'All fifteen tools are scoped to admitted stable TeamRun members. They read authoritative TeamRun and Charter-stage state, publish only public typed collaboration records, and mutate the compare-and-set task DAG; formation and ExpertBlueprint-bound child activation remain controller-owned.',
  },
  {
