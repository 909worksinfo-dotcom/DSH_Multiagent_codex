...
`ApprovalOutcome` 是闭合的，且失败时拒绝。`allowed-once` 仅授权所询问的那一个操作。`allowed-for-turn` 还会为工具名称与消费方定义的 `taskKey` 相同的请求安装内存授权，直到当前轮次结束；复用授权的请求仍会获得完整审计事件对。调用方对 `rejected`、`cancelled` 和 `unavailable` 均执行拒绝。缺失、不负责该请求、抛异常或不合规的应答者会产生 `unavailable`，而非授权。
